%  work        : Symphony No. 5 in F Major Op.76 (B.54)
%  typesetter  : Sébastien MANEN
%  date        : Wednesday 27 May 2026, 21:09
%###############################################################################
%#                          M U S I C   S E C T I O N                          #
%###############################################################################
musicCornoIVMvtI = \relative c {
	\clef treble
	\key c \major
	\transposition f
% Bars 1 to 5
	c'2\pp~
	c~
	\repeat volta 2 {
		c~
		c
		c~
% Bars 6 to 10
		c~
		c~
		c
		a\<
		<< g {s8 s s s\!}>>
% Bars 11 to 15
		g2\fz\>~
		g~
		g2~\pp
		g4. r8
		R2*4
% Bars 16 to 20
		
		
		
		R2*4
		
% Bars 21 to 25
		
		
		R2*2
		
		g'2\mf~
% Bars 26 to 30
		g8 r r g
		a2\fz~
		a4 r8 a
		g,2_\fbrackz~
		g4 r8_\crescmarkup g8
% Bars 31 to 35
		g2\fz~
		g4 r8 g
		g\f r r4
		f'4-> f8. c16
		f4\f r
% Bars 36 to 40
		f4-> f8. c16
		f8 r r4
		R2
		g,2_\pcresc~
		g~
% Bars 41 to 45
		g
		g8\f r \tuplet 3/2 4 {r8 g g
		g[ g g]  g\< g g
		g[ g g]  g g g\!} \markXoffset #-0.2 \mark \default
		c4\ff g'8-> gis-.
% Bars 46 to 50
		a-> aes-. g-> f-.
		e-. g,4-> g8~
		g g4 g8
		e'4-> g8-> gis8-.
		a-> aes-. g-> f-.
% Bars 51 to 55
		g8-. b,4-> b8~
		b b-. b-. g'-.
		c, r r4
		R2*5
		
% Bars 56 to 60
		
		
		
		\ni \mmrPos #-4 R2 \no
		b'8-.-\offset X-offset -1 \mf b,-. e-. r
% Bars 61 to 65
		R2
		r4 r8 b-.\p
		e-. r r4
		r4 r8 b-.\p
		e-. r r b-.
% Bars 66 to 70
		e-. r r b-.
		e-. r r_\dimmarkup b-.
		e-. r r b-.\pp
		e4 r
		R2*3
% Bars 71 to 75
		
		
		\ni \mmrPos #-4 R2
		\mmrPos #-4 R
		\mmrPos #-4 R
% Bars 76 to 80
		\mmrPos #-4 R \no
		R2*5
% Bars 81 to 85
		
		r4 aes8->\f r
		r4 g8->\f r
		r4 g8\f r
		r4 ges8_\crescmarkup r
% Bars 86 to 90
		r4 fis8-> r
		r4 fis8-> r
		b,->\f b'-> b,-> b'->
		b,-> b'-> b,-> b'->
		b,-. b'-. b-. b-.
% Bars 91 to 95
		b-. b-. b-. b-.
		gis r r4 \clef bass
		e,2_\pdim~
		e~
		e\pp \mark \default
% Bars 96 to 100
		R2*12
% Bars 101 to 105
		
% Bars 106 to 110
		
		
		\clef treble \ni \mmrPos #4 R2
		\mmrPos #4 R
		\mmrPos #4 R \no \clef bass
% Bars 111 to 115
		<< \hairpinShorten #'(-0.5 . -1) e2->-\offset X-offset -1 \mf\<~ {s8 s s s\!}>>
		<< e2\> {s8 s s s\!}>>
		<< e2~\< {s8 s s s\!}>>
		<< e2~\> {s8 s s s\!}>>
		e4\p r \clef treble
% Bars 116 to 120
		R2*4
		
		
		
		f'2\pp
% Bars 121 to 125
		R
		f
		R2
		b,-\offset X-offset -2.4 \pp
		R
% Bars 126 to 130
		f'\pp
		R2*4
% Bars 131 to 135
		\startMeasureCount g,2\pp~
		g~
		g~
		g~
		g~
% Bars 136 to 140
		g \stopMeasureCount
		R2*2
		
		g'2\ff~
		g
% Bars 141 to 145
		R2*4
		
		
		\mark \default
		g2\ff~
% Bars 146 to 150
		g
		gis8 r r4
		R2*7
% Bars 151 to 155
		
		
		
		
		gis2\ff~
% Bars 156 to 160
		gis
		R2*8
		
		
		
% Bars 161 to 165
		
		
		
		
		d2~-\offset X-offset -1 _\pcrescmolto
% Bars 166 to 170
		d
		cis2\ff~
		cis4 r
		R2*4
		
% Bars 171 to 175
		
		
		r4 cis->\f
		a-> e8.-> a16
		<< cis2\fp\>~ {s8 s s s\!}>>
% Bars 176 to 180
		cis2~
		cis~
		cis
		a\p~
		a~
% Bars 181 to 185
		a~
		a
		a2\pp~
		a
		g~
% Bars 186 to 190
		g~
		g~
		g
		R2*4
		
% Bars 191 to 195
		
		
		\alternative {
			\volta 1 {
				c2\pp~
				c\laissezVibrer
			}
			\volta 2 {
				bes2~\pp
			}
		}
	}
% Bars 196 to 200
	bes
	bes~
	bes
	bes~
	bes~
% Bars 201 to 205
	bes~
	bes
	g~
	g~
	g~
% Bars 206 to 210
	g
	g~
	g
	b!~
	b
% Bars 211 to 215
	R2*2
	
	a'2(~\pp
	a
	d,8) r r4
% Bars 216 to 220
	R2*7
% Bars 221 to 225
	
	
	\ni \mmrPos #-4 R2
	\mmrPos #-4 R
	\mmrPos #-4 R
% Bars 226 to 230
	\mmrPos #-4 R \no \mark \default
	b2\pp
	cis
	cis~
	cis
% Bars 231 to 235
	R2*6
	
	
	
	
% Bars 236 to 240
	
	fis2->\f~
	fis8. d16-. b-.( d-. fis-. b-.)
	fis4-> fis8. d16
	fis4.-> d8
% Bars 241 to 245
	g2->\ff~
	g8 r r4
	R2*10
	
	
% Bars 246 to 250
	
% Bars 251 to 255
	
	
	\ni \mmrPos #-4 R2
	\mmrPos #-4 R
	\mmrPos #-4 R
% Bars 256 to 260
	\mmrPos #-4 R \no \mark \default
	b,2\pp~
	b~
	b~
	b
% Bars 261 to 265
	dis~
	dis~
	dis~
	dis
	gis,\pp~
% Bars 266 to 270
	gis
	gis~
	gis
	aes2\p~
	aes_\crescmarkup
% Bars 271 to 275
	r8 c-.\< r f-.
	r aes-. r c-.\!
	bes4->\f bes8.-> f16-.
	bes4-> r
	aes2->\f~
% Bars 276 to 280
	aes8 r r4
	g2->\f~
	g8 r r4
	fis4->\ff fis8.-> dis16-.
	fis8 r r4
% Bars 281 to 285
	a4->\f r
	a-> r
	a-> r
	g-> r
	fis2\ff~
% Bars 286 to 290
	fis~
	fis~
	fis~
	fis8 r r4
	R2 \mark \default
% Bars 291 to 295
	R2*7
% Bars 296 to 300
	
	
	\ni \mmrPos #-4 R2
	\mmrPos #-4 R \no
	bes,2\p
% Bars 301 to 305
	R2*4
	
	
	
	\clef bass r8 f4.->\fz~
% Bars 306 to 310
	f4 f
	bes2\f~
	bes~
	bes8 f4.\fz~
	f8 f4.->_\dimmarkup~
% Bars 311 to 315
	f8 f4.~
	f8 f4.~
	f8 f4.~
	f2
	r8 f4.\p\>~
% Bars 316 to 320
	f2\!
	f2\p~
	f~
	f~
	f \clef treble
% Bars 321 to 325
	R2*8
% Bars 326 to 330
	
	
	\mark \default
	c''2\mf~
	c8. g16\< e-. g-. c-. e-.\!
% Bars 331 to 335
	c4-> c8. g16
	c4.-> g8
	<< g2~ {s4 s\>}>>
	g8. e16 c-.\! e-. g-. c-.
	g4_\dimmarkup g8. e16
% Bars 336 to 340
	g4. e8
	a2\pp
	b
	e,2\fp~
	e
% Bars 341 to 345
	R2*2
	
	\startMeasureCount c2\pp~
	c~
	c~
% Bars 346 to 350
	c~
	c~
	c~
	c~
	c \stopMeasureCount
% Bars 351 to 355
	R2*2
	
	g'4->\f g8.-> d16
	f4.-> d8-.
	f4-> f8.-> c16
% Bars 356 to 360
	f4.-> c8-.
	g2\f~
	g8 r r4
	g2\fz~
	g8 r r4
% Bars 361 to 365
	f'4\f r
	f r
	f r
	f r
	\startMeasureCount g,2\p~
% Bars 366 to 370
	g~
	g~
	<< g~ {s4 s_\crescmarkup}>>
	g2~ \stopMeasureCount
	g8-. r \tuplet 3/2 4 {r8 g-.\f g-.
% Bars 371 to 375
	g-.[ g-. g-.]  g-._\crescmarkup g-. g-.
	g-.[ g-. g-.]  g-. g-. g-.} \mark \default
	\tempoXoffset #2 c4->\ff g'8-. gis-.
	a8 aes g f
	e ees d b
% Bars 376 to 380
	g2
	c4 g'8 gis
	a aes g f
	e8-. b'4-> b8~
	b b-. b-. b-.
% Bars 381 to 385
	c,\fp e g-> gis
	a-> aes g-> f
	e->\< ees d-> b
	c-.\! fis,-. g( a16 b
	c8) r r4
% Bars 386 to 390
	R2*15
% Bars 391 to 395
	
% Bars 396 to 400
	
% Bars 401 to 405
	\ni \mmrPos #-4 R2
	\mmrPos #-4 R \no
	b2->\fz
	e->\fz
	r8 b'4.\f
% Bars 406 to 410
	R2
	r8 b4.\f->
	R2*9
% Bars 411 to 415
	
% Bars 416 to 420
	\mark \default
	R2*10
	
	
	
% Bars 421 to 425
	
% Bars 426 to 430
	
	\ni \mmrPos #-4 R2
	\mmrPos #-4 R
	\mmrPos #-4 R
	\mmrPos #-4 R \no
% Bars 431 to 435
	g,2-\offset X-offset -3 _\piuf~
	g
	g~
	g~
	g4 r
% Bars 436 to 440
	R2*3
	
	
	f'2\p(
	ees)
% Bars 441 to 445
	f(
	ees) \clef bass
	bes,~
	bes~_\dimmarkup
	bes~\>
% Bars 446 to 450
	<< bes {s8 s s s\!}>> \clef treble
	R2*8
% Bars 451 to 455
	
	
	
	\markk
	e'4(-\offset X-offset -1 \mf\< g\!
% Bars 456 to 460
	<< f2)\> {s8 s s s\!}>>
	e4(\< g\!
	<< \hairpinShorten #'(0 . -2) f2)\> {s8 s s\! s-\offset X-offset -4 _\crescmarkup}>>
	e4(\f\< g\!
	<< f2)\> {s8 s s s\!}>>
% Bars 461 to 465
	e4(\< g
	f2)\!
	e2\ff
	fis->
	gis->
% Bars 466 to 470
	a->
	R2*5
% Bars 471 to 475
	
	r4 g8\f r
	c,2->\ff~
	c4 c4->
	c2->~
% Bars 476 to 480
	c
	c->~
	c
	c_\dimmarkup~
	c
% Bars 481 to 485
	\startMeasureCount c2\p~
	c~
	c~
	c~
	c~
% Bars 486 to 490
	c~
	c~
	c~ \stopMeasureCount
	c4 r
	R2*7
% Bars 491 to 495
	
% Bars 496 to 500
	
	c2\p~
	c4. r8
	R2*6
	
% Bars 501 to 505
	
	
	
	
	\once \ni R2\fermata \fine
}
