%  work        : Symphony No. 4 in D Minor Op.13 (B.41)
%  typesetter  : Sébastien MANEN
%  date        : Sunday 04 January 2026, 08:21
%###############################################################################
%#                          M U S I C   S E C T I O N                          #
%###############################################################################
musicCornoIIMvtI = \relative c {
	\clef treble
	\key a \minor
	\transposition f
% Bars 1 to 5
	\repeat volta 2 {
		\mmrnDown R2.*4
		
		
		
		e'2\p~ e8 r
% Bars 6 to 10
		R2.
		e2~ e8 r
		R2.
		c'2~_\crescmarkup c8 e,-.
		e8.\< c'16 c4 a8. c16\!
% Bars 11 to 15
		b4.\fz a8->\< b-> b->\!
		g4->\f\< a-> b->\!
		c2->\f~ c8 f,-.
		f8.-> c'16-> c4-> r
		c2\fz~ c8 f,-. 
% Bars 16 to 20
		f8.-> c'16-> c4-> r
		c2->~\fz c8 f,-.
		f4->\< g-> bes->
		a-> bes-> a->\!
		gis2\ff~ gis8 b!-.
% Bars 21 to 25
		b8.-> e16 e8 r r4
		r gis,8.-> b16 b8-.-> d-.->
		b-> gis-> r4 r 
		R2.*2
		\mark \default
% Bars 26 to 30
		\tempoXoffset #0.5 a2->\ff~ a8 r
		gis2\fz~ gis8 r
		d'2\fz~ d8 r
		c2\fz~ c8 r
		cis2\fz~ cis8 r
% Bars 31 to 35
		c!2\fz~ c8 r
		a2\fz~ a8 r
		gis8-.-> r gis-.-> r gis-.\brack\fz r
		gis->-. r gis-.-> r gis-.\fz r
		gis-. r gis4-. a-.
% Bars 36 to 40
		r a-. a-.
		gis8-.-> r gis-.-> r gis-.\fz r
		gis-.-> r gis-.-> r gis-.\fz r
		gis-. r gis4-.\> a-.\!
		r a-._\dimmarkup a-.
% Bars 41 to 45
		gis8-. r r4 r
		R2.*28
		
		
		
% Bars 46 to 50
		
% Bars 51 to 55
	
% Bars 56 to 60
	
% Bars 61 to 65
	
% Bars 66 to 70
		
		
		
		
		gis2.\p(
% Bars 71 to 75
		a4)\< a4. a8\!
		b2\> b4\!
		a4\< a4. a8\!
		b2\> b4\!
		a2.~\<
% Bars 76 to 80
		a4 g2\!
		c2.~\>
		<< c {s4 s s\!}>>
		c2\p r4
		a2\<~ a8 b\!
% Bars 81 to 85
		c4 r r
		a2\<~ a8 b\!
		c4 r r
		R2.*4
% Bars 86 to 90
		
		\mark \default
		c,4\f c4. c8
		c2 c4
		c4 c4. c8
% Bars 91 to 95
		c2 c4
		c2.
		g'
		<< { \once \stemUp bes2( a4~} {s8\> s s s s s\!}>>
		a4 f4. g8)
% Bars 96 to 100
		a2.~
		a2\< bes4\!
		a4-.\f a4.-> a8-.
		a2->( g4)
		f-. a4.-> a8-.
% Bars 101 to 105
		a2->( g4)
		f2.->\fz
		c'->\fz
		c->\fz
		e,->\fz
% Bars 106 to 110
		b'4-^ r r
		R2.*7
		
		
		
% Bars 111 to 115
		
		
		
		\alternative {
			\volta 1 {
				b2.\pp~
				b~
% Bars 116 to 120
				b~
				b
				b2-> r4
				R2.*9
% Bars 121 to 125
	
% Bars 126 to 130
				
				
			}
			\volta 2 {
				R2.*2
				
			}
		}
	}
	R2.*6
% Bars 131 to 135
	
% Bars 136 to 140
	des2->\ff~ des8 aes-.
	aes8.-> des16 des4-. r
	des2->~ des8 aes-.
	aes8.-> des16 des4-. r
	aes8.-> des16 des4-. r
% Bars 141 to 145
	aes8.-> des16 des4-. r
	R2.*8
% Bars 146 to 150
	
	
	
	
	<< \hairpinShorten #'(-0.5 . -0.5) cis,2.->-\offset X-offset -1 \fz\> {s4 s s8 s\!}>>
% Bars 151 to 155
	R2.
	<< \hairpinShorten #'(-0.5 . -0.5) cis2.-\offset X-offset -1 \fz\> {s4 s s8 s\!}>>
	R2.
	e\fz->
	f->\fz
% Bars 156 to 160
	e->\fz
	f\fz
	R2.*14
	
	
% Bars 161 to 165
	
% Bars 166 to 170
	
% Bars 171 to 175
	\mark \default
	f8-.\ff r f-. r e-.\fz r
	f-. r f-. r e-.\fz r
	f-. r f2\fz
	g4-. r g-.
% Bars 176 to 180
	f8-. r f-. r e-.\fz r
	f-. r f-. r e-.\fz r
	f r e2\fz
	r4 e-^ e-^
	a8-. r a-. r des-. r
% Bars 181 to 185
	a-. r a-. r des-. r
	a-. r r4 r
	R2.
	des2\f->~ des8 aes-.
	aes8.-> des16 des4-. r
% Bars 186 to 190
	des2\fz->~ des8 aes-.
	aes8. des16 des4-. r
	aes2.\fz~
	aes2 r4
	aes2.\fz~
% Bars 191 to 195
	aes2 r4
	aes2.\fz
	<< aes-> {s4 s\> s}>>
	aes2.->
	<< aes->~ {s4 s s\!}>>
% Bars 196 to 200
	aes4\p r r
	R2.*17
% Bars 201 to 205
	
% Bars 206 to 210
	
% Bars 211 to 215
	
	
	
	a2->\mf~ a8 f-.\<
	c8. g'16 g4-. r
% Bars 216 to 220
	a2->~ a8 f
	c8. g'16 g4-.\! r
	a2\ff~ a8 f-.
	f8. a16 a4-. r
	a2~ a8 f-.
% Bars 221 to 225
	a8. c16 c4-. r
	a2->~ a8 e-.
	e8. a16 a4-. r
	a2->~ a8 a-.
	a8. a16 a4-. r
% Bars 226 to 230
	a2->~ a8 a-.
	a8. a16 a4-. r \mark \default
	a4-> r r
	R2.*4
	
% Bars 231 to 235
	
	
	r4 e->\f e->
	r dis-> dis->
	R2.*5
% Bars 236 to 240
	
	
	
	
	r4 a'->\f a->
% Bars 241 to 245
	r gis-> gis->
	cis2.->\fz~
	cis4 cis2\fz
	cis2.\fz\<~
	cis4\! cis2\fz
% Bars 246 to 250
	d2.\fz\<~
	d4\! d2\fz
	bes2.\fz\<~
	bes4\! bes2\fz
	bes2.\fz\<~
% Bars 251 to 255
	bes4\! bes2\fz
	\time 2/4 c4-> r
	a-> r
	f-> r
	\time 3/4 d'->\ff r b->
% Bars 256 to 260
	r gis-> r
	e-> r r
	e-> r r \mark \default
	e\f r r
	R2.
% Bars 261 to 265
	a2->\ff~ a8 r
	gis2\fz~ gis8 r
	gis2\fz~ gis8 r
	a2\fz~ a8 r
	cis2\fz~ cis8 r
% Bars 266 to 270
	c!2\fz~ c8 r
	a2\fz~ a8 r
	gis8-. r gis-. r gis-.\fz r
	gis-. r gis-. r gis-.\fz r
	gis-. r gis4-. a-.
% Bars 271 to 275
	r a-. a-.
	gis8-. r gis-. r gis-.\fz r
	gis-. r gis-. r gis-.\fz r
	gis-. r gis4-. a-.
	r a-. a-.
% Bars 276 to 280
	gis-.\f r r
	R2.*15
% Bars 281 to 285
	
% Bars 286 to 290
	
% Bars 291 to 295
	
	b2.->-\offset X-offset -2 _\fzcresc 
	c2.\fz
	cis->\fz\<
	<< \markEO #'(0 . 2) dis->_\fzmarkup {s4 s s8 s\!}>> \mark \default
% Bars 296 to 300
	b4\f r r
	R2.*3
	
	
	b2->\ff fis8.-. b16-.
% Bars 301 to 305
	ais2->( g8) r
	cis4..-> ais16[ e8. cis16]
	dis2 r4
	dis'4..-> b16[ fis8. dis16]
	e2 r4
% Bars 306 to 310
	e'4..-> c!16[ g8. e16]
	c'2->\fz r4
	c2->\fz r4
	c2->\fz r4
	c4 r r
% Bars 311 to 315
	R2.*4
	
	
	
	c2->\ff r4
% Bars 316 to 320
	c2-> r4
	c2-> r4
	c2-> r4
	c2.\ff~
	c~
% Bars 321 to 325
	c4 r r
	R2.
	c4-.\ff r r
	R2.
	c4-.\ff r r
% Bars 326 to 330
	R2.
	cis4-.\ff r r
	R2.
	b!2.->\ff~
	b~
% Bars 331 to 335
	b4 r r
	R2.
	gis4\f r r
	R2.
	gis2.\ff~
% Bars 336 to 340
	gis2 r4
	gis4-. r r
	R2.
	gis2.\ff~
	gis2 r4 \mark \default
% Bars 341 to 345
	R2.*8
	
	
	
	
% Bars 346 to 350
	
	
	
	a4\pp r r
	cis2(->\> b4)\!
% Bars 351 to 355
	a r r
	cis2->(\> b4)\!
	a r r
	R2.
	a,2.^>\fp\>~
% Bars 356 to 360
	a~
	a4\! r r
	R2. \mark \default
	R2.*4
	
% Bars 361 to 365
	
	
	a'4(_\crescmarkup cis4. cis8-.)
	cis2( b4)
	a(\< cis4. cis8-.)
% Bars 366 to 370
	cis2( b4)\!
	a2.->\mf~
	a2 b4
	<< a2.~\<_\moltocresc {s4 s s8 s\!}>>
	a2 b4
% Bars 371 to 375
	cis2(->\< b4)
	cis2(-> b4)\!
	cis\f r r
	d r r
	bes r r
% Bars 376 to 380
	\time 2/4 a-.\< r
	b!-. r
	c-. r
	b-.\! r
	\time 3/4 g2.\ff~
% Bars 381 to 385
	g~
	g~
	g
	r4 g\fz r
	r f\fz r
% Bars 386 to 390
	r f\fz r
	r g\fz r
	r des'\brack\fz r
	r c\brack\fz r
	r c\brack\fz r
% Bars 391 to 395
	e,!2\ff~ e8 r
	R2.
	c'2\p c4
	b2~ b8 r
	R2.*2
% Bars 396 to 400
	
	b2\pp b4
	a2~ a8 r
	R2.*21
	
% Bars 401 to 405
	
% Bars 406 to 410
	
% Bars 411 to 415
	
% Bars 416 to 420
	
	
	
	
	\ni \mmrPos #-6 R2.
% Bars 421 to 425
	\ni \mmrPos #-4 R \no
	r4 b\f r
	\mmrLength #18 \mmrnDown R2.*2
	\markk
	R2.*4
% Bars 426 to 430
	
	
	
	c2\p~ c8 e,-.
	e8. c'16 c4-. r
% Bars 431 to 435
	c2~ c8 e,-.
	e8. c'16 c4-. r
	R2.*6
% Bars 436 to 440
	
	
	
	a2->\ff r4
	b2-> r4
% Bars 441 to 445
	des2-> r4
	c2-> r4
	e,!2-> r4
	a4-^ r r
	gis-^ r r
% Bar 446
	a-^ r r\fermata \fine
}
