%  work        : Symphony No. 4 in D Minor Op.13 (B.41)
%  typesetter  : Sébastien MANEN
%  date        : Sunday 04 January 2026, 08:21
%###############################################################################
%#                          M U S I C   S E C T I O N                          #
%###############################################################################
musicTrombonoIIIMvtII = \relative c {
	\clef bass
	\key bes \major
%	\transposition a
% Bars 1 to 5
	bes2\pp f4 r8 f
	bes4(\< ees) <<ees,2( {s8 s s\! s\>}>>
	aes4)\! ges2 ges4(
	f4) d!2\< d4
	ees!\! ees2\>~ ees8 c'\!
% Bars 6 to 10
	d2\>( g,)\pp
	c2 f,4.\> g8\!
	aes2 bes4(\< c)
	c4-> cis->\! d2\> 
	g,2.\dimD r4
% Bars 11 to 15
	b4(\pp c d) r8 d\<
	b4\! c2\< g4
	c4\! e,2->\mf\> g4
	c\! g4.\pp g8( bes4)
	ees,\< c'2 ees,8-> ees->\!
% Bars 16 to 20
	d2-\offset X-offset -0.5 _\dimmarkup << \hairpinShorten #'(0 . -1) f\> {s8 s s s\!}>>
	a4(\p\> d c f,)\! \mark \default
	bes4\pp r r2
	R1*16
% Bars 21 to 25
	
% Bars 26 to 30
	
% Bars 31 to 35
	
	
	
	\mark \default
	\mmrLength #15 R1*17
% Bars 36 to 40
	
% Bars 41 to 45
	
% Bars 46 to 50
	
% Bars 51 to 55
	\mark \default
	\mmrLength #15 R1*17
% Bars 56 to 60
	
% Bars 61 to 65
	
% Bars 66 to 70
	
	
	
	\ni \mmrPos #-4 R1
	\mmrPos #-4 R \no \mark \default
% Bars 71 to 75
	R1*7
% Bars 76 to 80
	
	\mark \default
	R1*3
% Bars 81 to 85
	\ni \mmrPos #4 R1
	\mmrPos #4 R \no
	R1*2
	
	aes8-^\mf g-^ fis-^[ r16 fis16-.] b8-> bes->~ bes16\< aes-.-> bes-.-> ces-.->\!
% Bars 86 to 90
	bes4->\f c!-> des8-^ c-^ bes-^[ r16 bes-.]
	g'!8-> f->~ f16 e-.\< f-. fis-. g8-.-^ f-^ e!-^ d16-^ des-^\!
	c8-^\ff b-^ bes-^ r r2
	c8-^\ff b-^ bes-^ r r2
	R1
% Bars 91 to 95
	\tempoXoffset -2 R1*3
	
	\section
	\time 2/4 R2 \section \mark \default
	\time 4/4 \tempoXoffset #1 R1*7
% Bars 96 to 100
	
% Bars 101 to 105
	
	des1-^\mf
	<< \marcatoUpperSlur \hairpinShorten #'(0 . -1.5) des1-^~\> {s4 s s s\!}>>
	des8\p r r4 r2
	R1*4
% Bars 106 to 110
	
	
	
	des,1\pp-\alterBroken shorten-pair #'((-0.5 . 0)(0 . 1.3)) \>~
	des8 r\! r4 r2
% Bars 111 to 115
	\once \ni R1\fermata
	R1*2
	\mark \default
	\mmrCondens R1*7
% Bars 116 to 120
	
	
	
	
	\mark \default
% Bars 121 to 125
	R1*16
% Bars 126 to 130
	
% Bars 131 to 135
	
% Bars 136 to 140
	
	\ni \mmrPos #-4 R1
	\mmrPos #-4 R \no
	<< \hairpinShorten #'(-0.5 . -1) bes'1-\offset X-offset -1.5 \pp\<~ {s8 s s s \hairpinShorten #'(1 . -0.5) s\> s s s\!}>>
	\tempoXoffset #4 bes4 r r2
% Bars 141 and 142
	R1
	bes1\pp\fermata \fine
}
