%  work        : Symphony No. 4 in D Minor Op.13 (B.41)
%  typesetter  : Sébastien MANEN
%  date        : Sunday 04 January 2026, 08:21
%###############################################################################
%#                          M U S I C   S E C T I O N                          #
%###############################################################################
musicClarinettoIIMvtII = \relative c {
	\clef treble
	\key c \major
	\transposition bes
% Bars 1 to 5
	g'2\p g4-- r8 bes
	g4 a2\< a4\>
	bes4\! c2 f,4
	g4 gis2 b!8\< d
	c4(\! cis\> d4. f8)\!
% Bars 6 to 10
	e2\> << e {s4 s\pp }>>
	d2 cis
	e4(\< d) a4. b8
	b4-> b->\! a\> \tuplet 3/2 4 {b8( c d)\!}
	<< c2\> {s4 s_\dimmarkup}>> a4\! r8 a\pp
% Bars 11 to 15
	a2 a4 r8 a\<
	e'4(^\crescmarkup\! d2)\< \tuplet 3/2 4 {cis8( d e)\!}
	d4\mf << d2~ {s4 s\>}>> d8 cis(
	d4)\! f4(~\pp \tuplet 3/2 4 {f8 f e)} e4
	c2 d4_\crescmarkup e8-^ d-^ 
% Bars 16 to 20
	c4(\< << e2 {s4 s\!}>> \tuplet 3/2 4 {d8\> e b\!)}
	b2(_\p\> a4 b\pp) \mark \default
	c4 r r2
	R1*16
% Bars 21 to 25
	
% Bars 26 to 30
	
% Bars 31 to 35
	
	
	
	\mark \default
	R1*6
% Bars 36 to 40
	
% Bars 41 to 45
	\ni \mmrPos #-4 R1 \no
	R1*2
	
	\ni \ni \mmrPos #-4 R1 \no
	R1
% Bars 46 to 50
	r4 d'2(\pp \tuplet 3/2 4 {cis8\< d e}
	d4)\! fis->\fz(~ fis8[\> d]~ d cis\!
	d)\p r r4 r2
	R1*3
	
% Bars 51 to 55
	\mark \default
	c16_\ppbracklegato g8 g g g16~ g g8 g g g16~
	g c8 c\< a a16~ a a8 a\! a\> a16\!~
	a d8\p d c c16~ c c8 c c c16~
	c b8 b d d16~ d_\crescmarkup d8 d d d16~
% Bars 56 to 60
	d cis8 cis_\dimmarkup a a16~ a a8 a a a16~
	a\p g8 g g g16~ g g8 g g g16~
	g f8 f d' cis16~ cis\pp cis8 cis cis cis16~
	cis\< d8 d d d16\!~ d_\crescmarkup a8 a a a16~
	a\< a8 a a a16~ a c8 c16\!~ c b\>( c d~\!
% Bars 61 to 65
	d)\p c8 c_\dimmarkup c a16~ a a8 a a a16~
	a a8\pp a a a16~ a cis8 cis cis cis16~
	cis e8 e d d16~ d_\crescmarkup d8 d16~ d cis( d e~
	e)\< d8 d d d16\!~ d\> d8 d d cis16\!~
	cis d8 d c\pp c16~ c c8 c e e16~
% Bars 66 to 70
	e c8 c f f16~ f_\crescmarkup e8 d g f16~
	f c8\> c c c16~ c b8 b16~ b8\! r
	d2.\pp\> f4(\!
	d) r r2
	g,,2(\pp f \mark \default
% Bars 71 to 75
	e8) r r4 r2
	R1*6
% Bars 76 to 80
	
	\mark \default
	R1*2
	
	d'2(\pp f4 ees8 g
% Bars 81 to 85
	aes) r r4 r2
	R1*4
	
	
	
% Bars 86 to 90
	r4 fis->\f g-> c->
	c16 c c c  c\< c c c  c c c c  c c c cis\!
	d8\ffz r r4 r2
	R1*5
	
% Bars 91 to 95
	
	
	\section
	\time 2/4 R2 \section \mark \default
	\time 4/4 \tempoXoffset #1 R1*9
% Bars 96 to 100
	
% Bars 101 to 105
	
	
	
	\ni \mmrPos #-4 R1
	\mmrPos #-4 R \no
% Bars 106 to 110
	g,16\mp\< g g g  g g g g  aes aes aes aes  aes aes aes aes\! %g,2:16\mp\< << aes: {s4 s\!}>>
	g g g g  g g g g  aes\p\> aes aes aes   aes aes aes aes\! %g2: << aes:\p\> {s8 s s s\!}>>
	g8\pp r r4 r2
	r2 r8 aes,4.\pp(
	g8) r r4 r2
% Bars 111 to 115
	\once \ni R1\fermata
	R1*2
	\mark \default
	R1*7
% Bars 116 to 120
	
	
	
	
	\mark \default
% Bars 121 to 125
	\ni \mmrPos #4 R1
	\mmrPos #4 R \no
	e''2.\pp d8.(_\crescmarkup c16)
	c4(\< << b!2. {s4\! \hairpinShorten #'(0 . 1) s\> s8 s}>>
	c8)\! r r4 r b\p
% Bars 126 to 130
	<< \hairpinShorten #'(-0.7 . 0) b1->-\offset X-offset -2 \crescD\< {s4 s s s}>>
	c2(\f << des)\> {s8 s s s\!}>>
	c2\p\< << ees~ {s8 s s s\!}>> 
	ees4\< e\!(~ e8_\dimmarkup c4 b8)
	\hairpinShorten #'(0 . -1.5) c4(\p\< e~\! \once \tupletUp \tuplet 3/2 4 {e8\> c b} d4\!
% Bars 131 to 135
	c8) r e4(~\pp \tuplet 3/2 4 {e8 c b} d4
	c8) r r4 r2
	R1
	\ni \mmrPos #-4 R1
	\mmrPos #-4 R \no
% Bars 136 to 140
	R1*2
	
	<< c,1~\p\> {s4 s s s\!}>>
	c4 r r2
	\tempoXoffset #10 \ni \mmrPos #-4 R1 \no
% Bars 141 and 142
	c1\pp
	c\fermata \fine
}
