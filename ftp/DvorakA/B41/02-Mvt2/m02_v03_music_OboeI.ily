%  work        : Symphony No. 4 in D Minor Op.13 (B.41)
%  typesetter  : Sébastien MANEN
%  date        : Sunday 04 January 2026, 08:21
%###############################################################################
%#                          M U S I C   S E C T I O N                          #
%###############################################################################
musicOboeIMvtII = \relative c {
	\clef treble
	\key bes \major
%	\transposition a
% Bars 1 to 5
	R1*14
% Bars 6 to 10
	
% Bars 11 to 15
	
	
	
	
	\ni \mmrPos #6 R1
% Bars 16 to 20
	\mmrPos #4 R
	\mmrPos #4 R \no \mark \default
	bes''4(\p a aes) r8 aes(
	des4\< << c2) {s4\! s\>}>> \tuplet 3/2 4 {bes8( c des)\!}
	c4(\< << ees2) {s4\! s\>}>> bes8.( c16)\!
% Bars 21 to 25
	a4( d2)-\offset X-offset #-1 _\crescmarkup \tuplet 3/2 4 {d8(\< e fis)}
	<< g2~ {s8 s s s\!}>> g8 g(\> c16 bes a g)\!
	g4( f!\>~ f8) f\!( bes16\p a g f)
	f4( ees2)_\dimmarkup \tuplet 3/2 4 {d8( f ees)}
	d4(\pp\> c bes4. a8)\!
% Bars 26 to 30
	a4->\< a->\! bes4(\> d)\!
	d4.(\> bes8 g4)\! r8 g\pp
	d'4->( c\< b) r8\! g(
	f'4\crescD\< e2 \once \tupletUp \tuplet 3/2 4 {d8 e f\!}
	e4)\mf\< g4(~ \tuplet 3/2 4 {g8 e d\!} f4)\>
% Bars 31 to 35
	e(\! g\pp\>~ \tuplet 3/2 4 {g8 ees! d} f4)\!
	ees!( g\< f8->_\crescmarkup ees-> d-> c->)\!
	bes4(\< d2->\! \tuplet 3/2 4 {c8\> d a\!)}
	\hairpinShorten #'(-0.4 . -0.3) c2.(~-\offset X-offset -1.2 \p\> c8\pp bes) \mark \default
	bes4 r r2
% Bars 36 to 40
	R1*5
% Bars 41 to 45
	r2 aes'4(~\pp \tuplet 3/2 4 {aes4 g8)}
	f4( ees d4.\< c8)\!
	<< \hairpinShorten #'(-0.7 . 0.7) g2(\< {s8 s s s\!}>> << \hairpinShorten #'(-0.5 . -0.5) d2-\offset X-offset #-0.5 \fz\>~ {s8 s s s}>>
	d4)\pp r r2
	R1
% Bars 46 to 50
	r2 g'\p\<~
	<< g1~ {s4 s\! s\> s_\dimmarkup}>>
	g8\! r r4 r2
	R1
	\ni \mmrPos #-4 R1
% Bars 51 to 55
	\mmrPos #-4 R \no \mark \default
	bes,16_\ppbracklegato d8 d d d16~ d des8 des des des16~
	des f8 f\< g g16~ g g8 g\! g\> g16\!~
	g aes8\p aes ees ees16~ ees ees8 ees ees ees16~
	ees f8 f fis fis16~ fis_\crescmarkup fis8 fis fis a!16~
% Bars 56 to 60
	a g8 g_\dimmarkup g g16~ g g8 g16( c bes a g~
	g)\p f8 f f f16~ f f8 f16( bes a g f~
	f) ees8 ees ees ees16~ ees\pp ees8 ees16\(~ ees d f ees(
	d)\)\< c8 c c c16\!~ c_\crescmarkup d8 d ees ees16~
	ees\< ees8 ees e e16~ e f8 f16\!~ f fis(\> g a\!~
% Bars 61 to 65
	a)\p g8 g_\dimmarkup g d16~ d d8 d d d16~
	d f8\pp f f f16~ f f8 f f f16(
	g) g8 g g g16~ g_\crescmarkup g8 g g g16~
	g\< g8 g g g16\!~ g\> g8 g g g16~\!
	g g8 g g\pp g16~ g g8 g aes aes16~
% Bars 66 to 70
	aes g8 g g g16~ g_\crescmarkup g8 g g g16~
	g g8\> g g g16~ g f8 f f f16\!
	f2(\pp << ees {s4\> s\!}>>
	c4) r r2
	R1 \mark \default
% Bars 71 to 75
	bes4(\p^\solo a aes) r8 aes
	c4( << bes2 {\hairpinShorten #'(-0.5 . -0.5) s4\< s\!}>> \once \tupletUp \tuplet 3/2 4 {a!8\> bes c}
	bes4)\! ees2(-\tweak extra-offset #'(0 . -8) ^\p -\tweak extra-offset #'(0 . -8.3) ^\< c8. d16
	b4)\! d2(~ d16\< ees f g)
	g4(~\! g16 f ees d) c4(~_\dimmarkup c16 d ees f)
% Bars 76 to 80
	f4(\< fis g~\crescD g16 f ees d)
	c4(~\< c16 bes\! a g f4~ f8) r \mark \default
	R1*2
	
	aes4( a2 bes8 ees)
% Bars 81 to 85
	des2(\p d4_\crescmarkup ees8 f)
	ees4( e f8 g aes4~
	aes8) g-^ fis-^ r r2
	R1*2
	
% Bars 86 to 90
	r4 des8-^\f c-^ bes-^[ r16 bes-.] ges'8-> f~
	f16 e( f fis) g8-> f-> e-> r r4
	r r8 f(~\fz f16 e f g f8 e)
	r4 r8 f(~\fz\> f16 e f g f8 e)\!
	R1*4
% Bars 91 to 95
	
	
	\section
	\time 2/4 R2 \section \mark \default
	\time 4/4 \tempoXoffset #1 R1*2
% Bars 96 to 100
	
	g,1\pp(
	<< des'2\< {s8 s s s\!}>> << ces2)\> {s8 s s s\!}>>
	ees4--(\pp ees-- ees4.--) r8
	ees4--(\< ees-- ees4.--)\! r8 
% Bars 101 to 105
	ges4(\mf\< bes8 ees, ges[ c, ees bes16\! c]
	des8) r r4 r2
	R1
	f2.(\p ges8.\< aes16)
	ges4(\! ees2->)\> ees4(\!
% Bars 106 to 110
	f8) r r4 r2
	des8-.-^\mp c-.-^ ces-.-^ r16 beses\< beses8( ges'4\! \tuplet 3/2 8 {ees16\> f ges\!}
	f8)\p r r4 r2
	R1*2
	
% Bars 111 to 115
	\once \ni R1\fermata
	R1*2
	\mark \default
	bes,4(\pp a! aes) r8 aes(
	c4 bes2 \tuplet 3/2 4 {a!8 bes c}
% Bars 116 to 120
	bes4) ees2( c8. d16)
	b4( d2\<~ d16_\crescmarkup ees f g)\!
	g4(~ g16 f ees d c4_\dimmarkup~ c16 d ees f)
	f4(\p fis g~ g16\< f_\crescmarkup ees d
	c4~ c16 bes a g\! f4~ f8) r \mark \default
% Bars 121 to 125
	R1*2
	
	\ni \mmrPos #-4 R1
	\mmrPos #-4 R \no
	r4 \tuplet 3/2 4 {ees'8(\pp f ges} f) r \tuplet 3/2 4 {ees\p\<( f ges)\!}
% Bars 126 to 130
	\hairpinShorten #'(-0.5 . -1) ges2(\crescD\< f4 ees)\!
	<< des2.(\f~ {s4 s s\>}>> des8. ces16\!)
	<< bes2.(\p {s4 s\< s }>> a8. bes16)\!
	bes4(\< d!\! << f2~ {s4 s_\dimmarkup}>>
	f4)\p r r2
% Bars 131 to 135
	R1*3
	
	
	\tuplet 3/2 4 {r8\pp\< d(-- cis--} ees4\> d8)\! r r4
	R1
% Bars 136 to 140
	ges,8\fz r r4 r2
	bes'1\fp(
	f4 e2\> ees4\!
	d) r r2
	R1
% Bars 141 and 142
	d1\pp
	d\fermata \fine
}
