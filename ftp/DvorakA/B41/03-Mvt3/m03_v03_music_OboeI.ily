%  work        : Symphony No. 4 in D Minor Op.13 (B.41)
%  typesetter  : Sébastien MANEN
%  date        : Sunday 04 January 2026, 08:21
%###############################################################################
%#                          M U S I C   S E C T I O N                          #
%###############################################################################
musicOboeIMvtIII = \relative c {
	\clef treble
	\key d \minor
%	\transposition a
% Bars 1 to 5
	a'''4\ff r r r2 r4
	r2 r4 g2.->\fz
	aes->\fz g->\fz
	a!4 r r r2 r4
	r2 r4 g2.->\fz
% Bars 6 to 10
	aes->\fz g->\fz
	bes!4\brack\f r r r2 r4
	bes r r r2 r4
	bes\p r r r2 r8. c,,16(\<
	des4)\! r r r2 r8. e16(\<
% Bars 11 to 15
	f4)\! r r r2 r4
	r2 r8. \hairpinShorten #'(-0.5 . -1) g16(-\offset X-offset -1 \p\< e4)\! r r
	r2 r4 dis'(\< e) r
	fis( g) r a( bes) r
	a( bes)\! r a(\f bes) r
% Bars 16 to 20
	a,2\p bes4~ bes2 b4~
	b2_\pocoapococresc bes4~ bes2 a4~
	a2 bes!4~ bes2 b4~
	b2 bes4~ bes2 a4~
	\tempoXoffset #8 a2 bes4~ bes2 b4~
% Bars 21 to 25
	b2 c4~ c2 cis4-^\fz
	d1.-^\mf~
	d2 e4-. f-. e-. d-.
	<< c1.->( {s4\> s s s s s\!}>>
	a2.) r2 r8 a->
% Bars 26 to 30
	<< a1.\fz~ {s4 s s s\> s s}>>
	a2 a4-. c-. bes-. g-.\!
	a1.->~
	a2. r2 r8 a
	<< d1.~\fz\> {s4 s s s s s\!}>>
% Bars 31 to 35
	d2 e4-. f-. e-. d-.
	<< c1.->\>( {s4 s s s s s\!}>>
	\afterGrace 99/100 a1.) {\flag a8(}
	c1.)\fz\>~
	c2 d4-> e-> d-> b->\!
% Bars 36 to 40
	<< c1.->~ {s4 s s\> s s }>>
	c2.~ c4\! r r8 g
	bes!1.\fz\>~
	bes2 c4-. d-. c-. bes-.\!
	a1.->(
% Bars 41 to 45
	\afterGrace 99/100 e) {\flag e8(}
	a1.)->~
	a2 b4 c( b) gis-.
	a1.->~
	a2. r2 r8 a
% Bars 46 to 50
	bes!1.->\fz\>~
	bes2 c4-> d-> c-> bes->\!
	a1.->(
	\afterGrace 99/100 e) {\flag e8(}
	gis2)~ gis8 a b2~ b8 gis
% Bars 51 to 55
	a2~ a8\< b c2~ c8 a\!
	bes!1.\fz->~
	bes2 c4->_\crescmarkup d-> c-> bes->
	\afterGrace 99/100 a1.-> {\flag e8(}
	gis2)->~ gis8 a b2->~ b8 gis \mark \default
% Bars 56 to 60
	a'4\f r r r2 r4
	r2 r4 g2.->\fz
	aes->\fz g->\fz
	a!4\f r r r2 r4
	r2 r4 g2.->\fz
% Bars 61 to 65
	aes->\fz g->\fz
	r2 r4 \markEO #'(0 . 1.2) c!2.->\<_\fzmarkup 
	\markEO #'(0 . 1.45) des->_\fzmarkup \markEO #'(0 . 1.7) c->_\fzmarkup\!
	r2 r4 \markEO #'(0 . 1.2) aes2.->\<_\fzmarkup
	\markEO #'(0 . 1.45) aes->_\fzmarkup \markEO #'(0 . 1.7) aes->_\fzmarkup\!
% Bars 66 to 70
	r2 b4-._\pcresc b2-> b4-.
	r2 b4-. b2-> b4-.
	r2 b4-.\< b2-> b4-.
	b2-> b4-. b2-> b4-.\!
	a\f r a,\p~ a bes!2~_\crescmarkup
% Bars 71 to 75
	bes4 b2~ b4 bes2~
	bes4 a2~ a4 bes2~
	bes4 b2~ b4 bes2~
	\tempoXoffset #10 bes4 b2~ b4 c2~
	c4 cis2~ cis d4-. \section
% Bars 76 to 80
	\key d \major d1.->_\fmarcato~
	d2 e4-> fis-> e-> d->
	cis1.-^
	\afterGrace 99/100 a-^ {\flag a8}
	a1.->~
% Bars 81 to 85
	a2 b4-> cis-> b-> gis->
	fis1.-^~
	fis2. r2 a4-.
	d1.->~
	d2 e4-> fis-> e-> d->
% Bars 86 to 90
	cis1.->
	\afterGrace 99/100 a-> {\flag a8}
	cis1.->~
	cis2 cis4-> e-> d-> b->
	a1.->~
% Bars 91 to 95
	a2 r4 r2 r8 a
	b1.->\fz~
	b2 b4-> d-> cis-> b->
	a1.->\fz
	\afterGrace 99/100 fis1.-> {\flag fis8}
% Bars 96 to 100
	fis1.->\fz~
	fis2 fis4-> gis-> a-> gis->
	fis1.->~
	fis2. r2 r8 fis-.
	b1.->\fz~
% Bars 101 to 105
	b2 cis4-> d-> cis-> b->
	a1.->
	\afterGrace 99/100 fis-> {\flag fis8}
	eis2~ eis8 fis-. gis2~ gis8 eis-.
	fis2~ fis8 gis-. a2~ a8 fis-.
% Bars 106 to 110
	b1.->\fz~
	b2 cis4-> d-> cis-> b->
	\afterGrace 99/100 a1.-> {\flag fis8}
	eis2->~ eis8 fis gis2->~ gis8 eis \mark \default
	fis4 r r r2 r4
% Bars 111 to 115
	R1.
	r2 r8 ais-.\f\< cis2~ cis8\! gis
	ais2 r4 r2 r4
	r2 r4 g'2.->\mf\>
	g-> g->\!
% Bars 116 to 120
	r2 r4 g2.->\>
	g-> g->\!
	r2 r4 g2.->\p\>
	g-> g->\!
	r2 r4 g2.->\pp
% Bars 121 to 125
	g-> g->
	a1.\pp~
	a~
	a~
	a4 r r r2 r4
% Bars 126 to 130
	R1.*13
% Bars 131 to 135
	
% Bars 136 to 140
	
	
	
	\once \ni R1.\fermata \section \newSpacingSection
	\sectionLabel "TRIO" \time 2/4 \key c \major \tempoXoffset #1.5 R2*12
% Bars 141 to 145
	
% Bars 146 to 150
	
% Bars 151 to 155
	
	\ni \mmrPos #4 R2
	\mmrPos #4 R
	\mmrPos #4 R \no
	r4 \tuplet 6/4 4 {r16\brack\p\< g,( a b c d)\!} \markEsw #'(-0.3 . 0.3) \mark \default
% Bars 156 to 160
	e4~_\mffz\startTrillSpan e8\stopTrillSpan[ r16 d]
	e4~\fz\startTrillSpan e8[\stopTrillSpan r16 d]
	e4-> g->
	f-> e->
	d\fz~\startTrillSpan d8[\stopTrillSpan r16 cis]
% Bars 161 to 165
	d4\fz\startTrillSpan~ d8[\stopTrillSpan r16 cis]
	d4-> f!->
	e-> c->
	b4\fz\startTrillSpan~ b8[\stopTrillSpan r16 a]
	b4\fz\startTrillSpan~ b8[\stopTrillSpan r16 a]
% Bars 166 to 170
	b4-> c->
	b-> g->
	d'\fz\startTrillSpan~ d8[\stopTrillSpan r16 c]
	d4\fz\startTrillSpan~ d8[\stopTrillSpan r16 c]
	d4-> e->
% Bars 171 to 175
	d-> b16\< b32( c d e f g)\!
	a4\fz\startTrillSpan~ a8[\stopTrillSpan r16 g-.]
	a4\fz\startTrillSpan~ a8[\stopTrillSpan r16 g-.]
	a4-> c->
	b-> a->
% Bars 176 to 180
	g\fz\startTrillSpan~ g8[\stopTrillSpan r16 fis-.]
	g4\fz\startTrillSpan~ g8[\stopTrillSpan r16 fis-.]
	g4-> b->
	a-> g->
	f\fz\startTrillSpan~ f8[\stopTrillSpan r16 e-.]
% Bars 181 to 185
	f4\brack\fz\startTrillSpan~ f8[\stopTrillSpan r16 e-.]
	f4-> a-> \mark \default
	g8\fz r r4
	R2*7
% Bars 186 to 190
	
% Bars 191 to 195
	\tuplet 3/2 4 {g,8(\ff fis g)} g4(~
	\tuplet 3/2 4 {g8 fis g)} g4(~
	\tuplet 3/2 4 {g8 fis g) r gis( a)
	r ais( b) r b( c)
	b( ais b)} b4(~
% Bars 196 to 200
	\tuplet 3/2 4 { b8 ais b)} b4(~
	\tuplet 3/2 4 {b8 ais b) r b( a)
	r gis( b) r a( e)
	r d'( cis)} d4~
	\tuplet 3/2 4 {d8 d( cis)} d4(~
% Bars 201 to 205
	\tuplet 3/2 4 {d8 cis d) r dis( e)
	r cis( d!) r ais( b~
	b d fis)} fis4(~
	\tuplet 3/2 4 { fis8 eis fis)} fis4(~
	\tuplet 3/2 4 {fis8 eis fis) r eis( fis)
% Bars 206 to 210
	r fis( b)} g r
	\tuplet 3/2 4 {f,!( a c)} c4(~
	\tuplet 3/2 4 {c8 d a)} \shape #'((0 . -1)(0 . 0.5)(0 . 1)(0 . -1)) Slur c4(~
	\tuplet 3/2 4 {c8 d a c a' g~
	g f a,)} c4~
% Bars 211 to 215
	\tuplet 3/2 4 {c8 e( dis} e4~
	\tuplet 3/2 4 {e8 dis e)} e4(~
	\tuplet 3/2 4 {e8 dis e g e c
	a c f e c g)}
	f'4\brack\fz\startTrillSpan~ f8[\stopTrillSpan r16 e-.]
% Bars 216 to 220
	f4\brack\fz\startTrillSpan~ f8[\stopTrillSpan r16 e-.]
	f4-^ aes-^
	g-^ f-^ \markEsw #'(-0.3 . 0.3) \mark \default
	f4\brack\fz\startTrillSpan~ f8[\stopTrillSpan r16 e-.]
	f4\fz\startTrillSpan~ f8[\stopTrillSpan r16 e-.]
% Bars 221 to 225
	f4-^ aes-^
	g-^ f-^
	f8.->[\ff e16 f8.-> e16]
	f8-.[ aes-. g-. f-.]
	f8.[-> e16 f8.-> e16]
% Bars 226 to 230
	f8[-. aes-. g-. f-.]
	f-. r r4
	R2*14
% Bars 231 to 235
	
% Bars 236 to 240
	
% Bars 241 to 245
	
	\mmrLength #33 \mmrnDown R2*15
% Bars 246 to 250
	
% Bars 251 to 255
	
% Bars 256 to 260
	
	\mmrLength #20 \mmrnDown R2*7
% Bars 261 to 265
	
	
	
	\ni \mmrPos #-4 R2
	\mmrPos #-4 R \no \section \newSpacingSection
% Bars 266 to 270
	\time 6/4 \key d \minor a4\ff r r r2 r4
	r2 r4 g2.->\fz
	aes->\fz g->\fz
	a!4 r r r2 r4
	r2 r4 g2.->\fz
% Bars 271 to 275
	aes->\fz g->\fz
	bes!4\brack\f r r r2 r4
	bes r r r2 r4
	bes\p r r r2 r8. c,,16(\< 
	des4)\! r r r2 r8. e16(\<
% Bars 276 to 280
	f4)\! r r r2 r4
	r2 r8. \hairpinShorten #'(-0.3 . -1) g16(-\offset X-offset -1 \p\< e4)\! r r
	r2 r4 dis'(\< e) r
	fis( g) r a( bes) r
	a( bes)\! r a(\f bes) r
% Bars 281 to 285
	a,2\p bes4~ bes2 b4~
	b2_\pocoapococresc bes4~ bes2 a4~
	a2 bes!4~ bes2 b4~
	b2 bes4~ bes2 a4~
	\tempoXoffset #9 a2 bes4~ bes2 b4~
% Bars 286 to 290
	b2 c4~ c2 cis4-^\fz
	d1.-^\mf~
	d2 e4-. f-. e-. d-.
	<< c1.->( {s4\> s s s s s\!}>>
	a2.) r2 r8 a->
% Bars 291 to 295
	<< a1.\fz~ {s4 s s s\> s s}>>
	a2 a4-. c-. bes-. g-.\!
	a1.->~
	a2. r2 r8 a
	<< \hairpinShorten #'(-0.5 . -1) d1.\fz\>~ {s4 s s  s s s\!}>>
% Bars 296 to 300
	d2 e4-. f-. e-. d-.
	<< c1.->\>( {s4 s s s s s\!}>>
	\afterGrace 99/100 a1.) {\flag a8(}
	c1.)\fz\>~
	c2 d4-> e-> d-> b->\!
% Bars 301 to 305
	<< c1.->~ {s4 s s\> s s s}>>
	c2.~ c4\! r4 r8 g
	bes!1.\fz\>~
	bes2 c4-. d-. c-. bes-.\!
	a1.(->
% Bars 306 to 310
	\afterGrace 99/100 e) {\flag e8(}
	a1.->)~
	a2 b4 c( b) gis-.
	a1.->~
	a2. r2 r8 a
% Bars 311 to 315
	bes!1.->\fz\>~
	bes2 c4-> d-> c-> bes->\!
	a1.(->
	\afterGrace 99/100 e) {\flag e8(}
	gis2)~ gis8 a b2~ b8 gis
% Bars 316 to 320
	a2~ a8\< b c2~ c8 a\!
	bes!1.->\fz~
	bes2 c4->_\crescmarkup d-> c-> bes->
	\afterGrace 99/100 a1.-> {\flag e8(}
	gis2->)~ gis8 a b2->~ b8 gis \markXoffset #-0.2 \mark \default
% Bars 321 to 325
	a'4\f r r r2 r4
	r2 r4 g2.->\fz 
	aes->\fz g->\fz
	a!4\f r r r2 r4
	r2 r4 g2.->\fz
% Bars 326 to 330
	aes->\fz g->\fz
	r2 r4 \markEO #'(0 . 1.2) c!2.->\<_\fzmarkup
	\markEO #'(0 . 1.45) des->_\fzmarkup \markEO #'(0 . 1.7) c->_\fzmarkup\!
	r2 r4 \markEO #'(0 . 1.2) aes2.->_\fzmarkup\<
	\markEO #'(0 . 1.45) aes->_\fzmarkup \markEO #'(0 . 1.7) aes->_\fzmarkup\!
% Bars 331 to 335
	r2 b4-._\pcresc b2-> b4-.
	r2 b4-. b2-> b4-.
	r2 b4-.\< b2-> b4-.
	b2-> b4-. b2-> b4-.\!
	\dynEO #'(0 . 1) a\f r a,\p~ a bes!2_\crescmarkup~
% Bars 336 to 340
	bes4 b2~ b4 bes2~
	bes4 a2~ a4 bes2~
	bes4 b2~ b4 bes2~
	\tempoXoffset #10 bes4 b2~ b4 c2~
	c4 cis2~ cis d4-. \section \newSpacingSection
% Bars 341 to 345
	\sectionLabel "CODA" \time 2/4 \key d \major fis4_\fffz->~ fis8[ r16 e-.]
	fis4->\fz~ fis8[ r16 e-.]
	fis4-> a->
	g-> fis->
	e4.->\fz r16 dis-.
% Bars 346 to 350
	e4.->\fz r16 dis-.
	e4-> g->
	g-> g->
	fis4.->\fz r16 fis-.
	fis4.\fz r16 fis-.
% Bars 351 to 355
	fis4-> fis->
	fis-> g->
	fis8 r r4
	R2*11
% Bars 356 to 360
	
% Bars 361 to 365
	
	
	
	
	\tempoXoffset #-2 \mmrLength #20 R2*10
% Bars 366 to 370
	
% Bars 371 to 375
	
	
	
	
	a,4_\ppcrescpoco a
% Bars 376 to 380
	a\< a 
	a a\!
	a a
	d-> a'->
	g-> f->
% Bars 381 to 385
	a-> f->
	d'->\< a->\!
	f'8\fz r r4
	R2*3
	
% Bars 386 to 388
	
	r4 \tuplet 3/2 4 {r8 r a,-.\ff}
	a8-. r r4\fermata \tweak direction #-1 \textEndMark "Fine" \fine
}
