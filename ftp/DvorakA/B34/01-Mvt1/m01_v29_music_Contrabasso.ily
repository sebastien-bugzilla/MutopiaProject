%  work        : Symphony No. 3 in E-flat Major Op.10 (B.34)
%  typesetter  : Sébastien MANEN
%  date        : Tuesday 29 July 2025, 23:43
%###############################################################################
%#                          M U S I C   S E C T I O N                          #
%###############################################################################
musicContrabassoMvtI = \relative c {
	\clef bass
	\key ees \major
%	\transposition a
% Bars 1 to 5
	ees4\pp^\pizz r8 ees4 r16 bes
	ees4 r8 ees4 r16 bes
	ees4 r8 ees4 r16 bes
	ees4 r8 ees4 r16 bes
	d4\< r8 c4 r16 g\!
% Bars 6 to 10
	c4\fz r8 bes4.\fz^\arco~
	bes\> aes\pp
	g4 r8 g4-. r16 ees-.
	<< \hairpinShorten #'(-0.5 . -1) aes2.\fp\> {s8 s s  s s s\!}>>
	g4 r8 g4 r16\< ees\!
% Bars 11 to 15
	<< \hairpinShorten #'(-0.5 . 0) aes2.\fz\> {s8 s s  s s s }>>
	g2.\p
	ges_\crescmarkup
	ces->\fz
	<< bes {\hairpinShorten #'(0 . -0.5) s4.\> s8 s s\!}>>
% Bars 16 to 20
	ees4.\ff-^ r4 r8
	bes4.-^ r4 r8
	aes4.-^ r4 r8
	\time 3/8 ges4.:32-^\ff
	\time 3/4 f'8-^ r ees-^ r d-^ r
% Bars 21 to 25
	\time 6/8 d->\< ees-> f-> ees-> f-> g->\!
	\time 3/4 f-^ r ees-^ r d-^ r
	\time 6/8 ees->\< f-> g-> aes-> a-> bes->\! \mark \default
	c,4.\fz r4 r8
	c4.\fz r4 r8
% Bars 26 to 30
	<< des2.\> {s8 s s  s s s\!}>>
	ges,4\p r8 r4 r8
	R2.*4
	
	
% Bars 31 to 35
	
	aes4.:32\ff aes8 aes4->
	aes4->\< aes8->~ aes aes-> aes->\!
	aes4.:32->\ff aes8 aes4->
	aes4->\< aes8->~ aes aes-> aes->\!
% Bars 36 to 40
	bes4.->\ff ees->
	aes,-> des->
	\time 3/4 ges,8-> r aes-> r bes-> r
	\time 6/8 aes8->\< bes-> ces-> des4(-> ges,8)\!
	\time 3/4 ges-> r aes-> r bes-> r 
% Bars 41 to 45
	\time 6/8 aes->\< bes-> ces-> c-> des-> d->\!
	\time 3/8 ees->\< f-> ges->\! \mark \default
	\time 6/8 \hairpinShorten #'(0 . -1) g,!4\fz\< g'!8->\! r4 r8
	\hairpinShorten #'(0 . -1) g,4\fz\< g'8->\! r4 r8
	\hairpinShorten #'(0 . -1) aes,4\fz\< aes'8->\! r4 r8
% Bars 46 to 50
	\hairpinShorten #'(0 . -1) a,4\fz\< a'8->\! r4 r8
	a,4-> a'8-> a,4-> a'8->
	a,4-> a'8-> a,4-> a'8->
	a, a'-> a, a'-> a, a'->
	bes,4->\< bes8->~ bes bes4->\!
% Bars 51 to 55
	bes8\< d16 f bes-> aes-> g-> f-> ees-> d-> c->\! bes-> \mark \default
	ees4->_\ffmarcatissimo r8 ees4-> r16 bes->
	ees4-> r8 ees4-> r16 bes-.
	d4->\< r8 c4-> r16 g-.\!
	c4-> r8 bes4.\ffzD\>~
% Bars 56 to 60
	bes\! aes4-> r16 ees-.
	g4->\f\< r16 ees-. g4-> r16 ees-.\!
	aes4.\fz~ aes4 r16 ees->
	g4-> r16 ees-.\< g4-> r16 ees-.\!
	aes4.\fz~ aes4 r16 ees
% Bars 61 to 65
	g4.->~ g4 r8
	ges4.->~ ges4 r8
	f2.->
	ges4.->~ ges4 r8
	g!4.->~ g4 r8
% Bars 66 to 70
	aes4.->~ aes4 r8
	a4_\fffmarcatissimo c16[ r32 b] b8-> d-> c->
	ees16->[\< r32 d d8-. fis16-> ees->] a-> fis-> c'-> a-> ees'-> c->\!
	fis8-^ r r r4 r8
	R2.*2
% Bars 71 to 75
	
	r8 a,,16-.\pp bes-. c8-. c16[-. d-. ees8-.] ees16[-. f-.
	ges8-.] r r r4 r8
	r c,16-. des-. ees8-. ees16[-. f-. ges8-.] c,16-. des-.
	ees8-.\< f-. ges-. a-. bes-. c-.\!
% Bars 76 to 80
	des r\> r ces[^\pizz r aes!8]\! \mark \default
	ges4\pp r8 r4 r8
	bes,4 r8 r4 r8
	aes4 r8 r4 r8
	ces4 r8 r4 r8
% Bars 81 to 85
	bes4 r8 r4 r8
	ces4_\brackM\crescmarkup r8 r4 r8
	des4_\brackM\dimmarkup r8 des4 r8
	ges,4.\pp^\arco~ ges4\<~ ges16. aes32\!
	bes4.\>~ bes4\!~ bes16. aes32
% Bars 86 to 90
	aes4.~ aes4\<~ aes16. bes32\!
	ces4.->\>~ ces4\!~ ces16. bes32
	<< bes2.\< {s8 s s  s s s\!}>>
	<< ces2.\> {s8 s s  s s s\!}>>
	des2.\pp
% Bars 91 to 95
	ges,_\pocoapoco_cresc
	ces
	<< des\< {s8 s s  s s s\!}>>
	ees4.-^ ces-^
	des2.
% Bars 96 to 100
	ges,4.-^\ff r4 r8
	ges4.-^ r4 r8
	ges'4.-^ r4 r8
	f,4.->\ff~ f8 f4->
	f4-> f-> f->
% Bars 101 to 105
	f4.->~ f8 f4->
	f4-> f-> f->
	ees'8-. r r r4 r8
	r ees16-.\pp[ f-. ges8-. a-.] r r
	r a,16-.\p c-. ees8-. f-. a-. c-.
% Bars 106 to 110
	f-. r r r4 r8
	r aes,,!16[-._\ppocoa_pococresc c-. des8-. f-.] r r
	r aes,16-.[ c-. des8-. f-.] r r
	r b,16-.\<[ d!-. f8-. aes-.]\! r r
	r d,16-.\<[ f-. aes8-. ces-.]\! r r
% Bars 111 to 115
	bes,\f r r bes\< r r
	bes r r bes r r\!
	bes4\ff r8 r4 r8
	R2.*4
	
% Bars 116 to 120
	
	
	ees4\pp^\pizz r8 r4 r8
	ees4 r8 r4 r8
	R2.*3
% Bars 121 to 125
	
	
	des4\p r8 des4 r16 des
	des4\< r8\! des4\> r8\!
	ges,4 r8 r4 r8
% Bars 126 to 130
	R2.
	aes4\pp^\arco r8 r4 r8
	R2.
	<< \hairpinShorten #'(-0.5 . -1) gis2.\fp\>~ {s8 s s  s s s\!}>>
	<< gis2.\< {s8 s s  s s s\!}>>
% Bars 131 to 135
	a2.\fp\<~
	a4.\! << c {\hairpinShorten #'(0 . -1) s8\> s s\!}>>
	b4 r8 r4 r8
	b4_\dimmarkup r8 b4 r8
	e4\brack\p r8 r4 r8
% Bars 136 to 140
	R2.*3
	
	
	<< g,2.\mf~ {s4. s8\< s s}>>
	<< {g4. a4( fis8)} {s16 s s\! s s s\>  s s s s s s\!}>>
% Bars 141 to 145
	<< b2.( {s4.\< s8 s s\!}>>
	<< d2.)\> {s8 s s  s s s\!}>>
	g,4.\pp~ g8 r r
	g4.~ g8 r r
	a4.~ a8 r r
% Bars 146 to 150
	a4.~ a8 r r
	a2.:32\fp\<
	<< a: {s8 s s  s s s\!}>>
	bes2.:\ff
	bes4.:\> << des: {s8 s s\!}>>
% Bars 151 to 155
	c4\p r8 r4 c16 c
	c4\> r8 r4 c16 c\!
	des4.(_\ppespress c
	bes aes)
	des( c
% Bars 156 to 160
	bes aes)
	ees'4 des8~ des\<^\brackM\pococresc c-. bes-.\!
	f'4 ees8~ ees des-. c-.
	ges'4-^ f-^ ees-^
	\time 3/4 d!8-^\f r ees-^ r f-^ r
% Bars 161 to 165
	\textMark "Vi-" \time 6/8 d-^\< ees-^ f-^ g!-^\! aes-^\> g-^\!
	\time 3/4 f-^ r ees-^ r d-^ r \textEndMark "-de"
	\time 6/8 d-^ ees-^ f-^ g-^\! aes-^ bes-^
	c,4.\fz r4 r8
	c4.\fz r4 r8
% Bars 166 to 170
	cis4.\fz r4 r8
	dis4.\fz r4 r8 \mark \default
	e8-.\brack\fz r r r4 r8
	R2.*2
	
% Bars 171 to 175
	<< f,2.\f~ {s4.\> s8 s s\!}>>
	f4.~ f4 r8
	e'4\p^\pizz r8 r4 r8
	R2.*2
	
% Bars 176 to 180
	f,2.\f^\arco~
	f4.\>~ f4\! r8
	dis'4^\pizz_\pbrackp r8 r4 r8
	R2.*2
	
% Bars 181 to 185
	e2.\f^\arco~
	e4.~\> e8-\offset X-offset #1.5 \dimD r r
	ees!4\pp^\pizz r8 r4 r8
	R2.*2
	
% Bars 186 to 190
	g8\fz^\arco-^ g16-^ g-^ g8-^ r4 r8
	f,2.\fz
	g'8-^\fz g16-^ g-^ g8-^ r4 r8
	f,2.\fz
	d''8\fz d16 d d8 r4 r8
% Bars 191 to 195
	cis,2.\fz
	d'8-^\fz d16-^ d-^ d8-^ r4 r8
	cis,2.\fz
	des'4.-^\ff bes-^
	des-^ bes-^
% Bars 196 to 200
	des4-^ bes8-. des4-^ bes8-.
	des4-^ bes8-. r4 r8
	R2.*7
	
	
% Bars 201 to 205
	
	
	
	
	d,!8-^\p^\pizz r r r4 r8
% Bars 206 to 210
	R2.*3
	
	
	r4 r8 aes'4-^\p r8 
	R2.
% Bars 211 to 215
	ees4\fz-^ r8 r4_\crescpocoapoco r8
	R2.
	a4-^ r8 r4\< r8
	e4 r8 r4 r8
	b'4 r8 r4 r8\! \mark \default
% Bars 216 to 220
	d,2.:32\ff^\arco
	d:
	d:
	d: 
	d4-^\fz r8 d4-^\fz r8 
% Bars 221 to 225
	d4-^\fz r8 d4-^\fz r8 
	d4-^\brack\fz r8 d4-^\brack\fz r8 
	d4-^ d-^ d-^
	bes2.:32\fff
	bes8 r r bes r r
% Bars 226 to 230
	\time 3/4 bes-. r bes-. r bes-. r
	\time 6/8 << bes2.~\> {s8 s s  s s s\!}>>
	bes4 r8 r4 r8
	\time 3/8 << \hairpinShorten #'(-0.5 . -1) \noteHeadEsw #'(0 . 2.5) bes4.(\p\> {s8 s s\!}>>
	\time 6/8 \revertNoteHeadEsw \tempoXoffset #7 ees4)\p r8 ees4^\pizz r16 bes
% Bars 231 to 235
	ees4 r8 ees4 r16 bes
	d4 r8 c4\< r16 g\!
	c4\fp\>^\sempredim r8 c4 r16 g\!
	bes4 r8 aes4 r8
	g4.\p~^\arco g4 ees8
% Bars 236 to 240
	aes2.
	g\pp(
	aes)
	g^\pocoapococrescendo
	ges
% Bars 241 to 245
	f(
	ges)
	<< g!( {s4. s8\< s s}>>
	<< aes2.) {s8 s s  s s s\!}>>
	a4-^\f c16[ r32 b] b8-^ d-^ c-^
% Bars 246 to 250
	ees!-^\< d-^ fis16 ees a-> fis-> c'-> a-> ees'-> c->\!
	fis8-.\fz r r r4 r8
	dis\fz r dis,16(\p cisis dis8) fis-. b-.
	d!-^\fz r r r4 r8
	des-^ r des,16(\p c des8) f!-. aes!-.
% Bars 251 to 255
	b-.\brack\fz r r r4 r8
	bes!-.\fz r bes,16(\p a bes8) d-. g-.
	a-.\brack\f\< r r g-. r r
	fis-. r r f-. r r\! \mark \default
	e\ff r r r4 r8
% Bars 256 to 260
	R2.*2
	
	e8\fz^\pizz r r r4 r8
	R2.*4
	
% Bars 261 to 265
	
	
	fis8\p r r r4 r8
	R2.*6
	
% Bars 266 to 270
	
	
	
	
	b,8\pp r r r4 r8
% Bars 271 to 275
	b'4 r8 r b b
	b,4 r8 r4 r8
	b'4 r8 r b b 
	bes!4\ppp r8 r bes bes
	bes4_\pocoapococresc r8 r bes bes
% Bars 276 to 280
	a4 r8 r a a
	a4 r8 r a a
	g,2.:32\mf\<^\arco
	g:
	<< g: {s8 s s  s s s\!}>>
% Bars 281 to 285
	g8\f-^ aes-^ a-^ bes-^[ b-^ r16 c-.]
	\hairpinShorten #'(0 . 2.5) c8-^\<_\molto cis-^ d-^ ees16-^ e-^ f-^ fis-^ g-^ aes!-^\!
	a8-^_\brackfffz r r r4 r8
	aes,!8\fz r r r4 r8
	aes'\fz r r r4 r8
% Bars 286 to 290
	g,\fz r r r4 r8
	g4 r8 g4 r8
	g4 r8 r4 r8
	f8\fz r r r4 r8
	f-> r r r4 r8
% Bars 291 to 295
	g\fz r r r4 r8
	g-> r r r4 r8
	aes!\brack\fz r r r4 r8
	aes-> r r r4 r8
	ces4\fz r8 r4 r8
% Bars 296 to 300
	ces4->-\offset X-offset -1 _\pocoapoco_crescendo r8 r4 r8
	bes4\fz r8 bes'4-> r16 bes,
	bes4-> r8 bes'4-> r16 bes,
	bes4-> r8 bes'4-> r16 bes,
	bes4-> r8 bes'4-> r16 bes,
% Bars 301 to 305
	bes8-^^\marcatiss c-^ cis-^ d-^\< ees-^ f16-^ g-^
	aes8-> a-> bes16-> a-> bes-> b-> c-> d-> ees-> e->\!
	f8 r r e-^ r r
	d-^ r r des-^ c-^ bes-^
	ees,4.-^\ff d16( ees f ees d ees)
% Bars 306 to 310
	bes'4.~ bes4 ees,16-. f-.
	g8\<-^ aes-^ bes-^ c-^ d-^ ees-^\!
	ees4.\ff~ ees8. d16-^ f-^ ees-^
	d4.-^ c-^
	bes8\fz r r r4 r8
% Bars 311 to 315
	bes,\fz r r r4 r8
	bes\fz r r r4 r8
	bes-. r r bes-. r r
	bes-. r r bes-. r r
	bes2.:32_\pbrackcresc
% Bars 316 to 320
	bes4.: bes8 r r
	ees4.-^\ff r4 r8
	bes4.-^ des-^
	ges,-^ f-^
	ees'-^ d16( ees f ees d ees)
% Bars 321 to 325
	bes'4~ bes16.\< ces32-. des4.-^\!
	ges,-^ bes8 r r
	ees,-^ r r bes-^ r r
	ees-^ r r bes-^ r r
	bes-^ r r bes-^ r r 
% Bars 326 to 330
	bes-^ r r bes-^ r r 
	R2.
	r4 r8 bes16\f-^ bes-^ bes-^ bes-^ bes-^ bes-^ 
	R2.
	r4 r8 bes16-^\ff bes-^ bes-^ bes-^ bes-^ bes-^ 
% Bars 331 to 335
	R2.*3
	
	
	d8-^\mf\< ees-^ bes-^ d-^ ees-^ bes-^ 
	d-^ ees-^ bes-^ d-^ ees-^ bes-^\!
% Bars 336 to 340
	d4.\f-^ ees-^
	ces4.:32 aes8-^ aes-^ aes-^
	ees'2.:32\ff
	ees4 r8 r4 r8
	ees4 r8 r4 r8
% Bars 341 to 343
	ees4 r8 r4 r8
	ees4 r8 r4 r8
	ees2.\ff\fermata \fine
}
