%  work        : Symphony No. 3 in E-flat Major Op.10 (B.34)
%  typesetter  : Sébastien MANEN
%  date        : Tuesday 29 July 2025, 23:43
%###############################################################################
%#                          M U S I C   S E C T I O N                          #
%###############################################################################
musicFlautoIMvtI = \relative c {
	\clef treble
	\key ees \major
%	\transposition a
% Bars 1 to 5
	R2.*11
% Bars 6 to 10
	
% Bars 11 to 15
	
	f''4(\brack\p\< des'16)[\! r32 c-.] c4( bes8)
	bes4(\<_\crescmarkup ges'16)[\! r32 f-.] f4( ees8)
	ees4.(~\brack\fz ees16 d f ees ces aes)
	ges4._\dimmarkup f4(\brack\p bes8)
% Bars 16 to 20
	ees,4.->\ff d16( ees f ees d ees)
	\dynEO #'(0 . 2) bes'4.-^\fz f16( ges aes ges f ges)
	\dynEO #'(0 . 2) des'4.-^\fz c16( des ees des c des)
	\time 3/8 \dynEO #'(0 . 3) bes'4.->\ff
	\time 3/4 a,16(-> bes c bes) a->( bes c bes) a(-> bes c bes)
% Bars 21 to 25
	\time 6/8 a(\< bes) c( bes) bes( aes) aes( g) g( f) f( ees)\!
	\time 3/4 a(-> bes c bes) a->( bes c bes) a(-> bes c bes)
	\time 6/8 a(\< bes) c( bes) bes( aes) aes( g) g( f) f( ees)\! \mark \default
	\dynEO #'(0 . 2) ees'4.\brack\fz d16( ees f ees d ees)
	\dynEO #'(0 . 2) ees4.\brack\fz d16( ees f ees d ees)
% Bars 26 to 30
	des4.(\> c16 des f ees des f,)\!
	ges4.(\p f16 ges aes ges f ges)
	des'4.~ des4 ges,16-. aes-.
	bes8->\< ces-> des-> ees->[ f-> r16 ges]\!
	ges4.( f16\< ges aes ges f ges)\!
% Bars 31 to 35
	ges4.(_\brackM\crescmarkup f16 ges aes ges f ges)
	\dynEO #'(0 . 3) bes4.->\ff~ bes8 aes4->
	ges4->\< f8->~ f ges-> aes->\!
	\dynEO #'(0 . 3) bes4.\ff->~ bes8 aes!4->
	ges->\< f8->~ f ges-> aes->\!
% Bars 36 to 40
	\dynEO #'(0 . 2) ces,4(\fz-> bes8) a16( bes ces bes a bes)
	\dynEO #'(0 . 2) f'4->(\fz ees8) g,16( aes! bes aes g aes)
	\time 3/4 ees'8( des16) r ees8( des16) r ees8( des16) r
	\time 6/8 ees(\< des) ges( f) f( ees) ees( des) des( ces) a( bes)\!
	\time 3/4 ees8( des16) r ees8( des16) r ees8( des16) r
% Bars 41 to 45
	\time 6/8 ees(\< des) aes'( ges) ges( f) f( ees) ees( des) des( ces)\!
	\time 3/8 ces(\< bes) bes( aes) aes( ges)\! \mark \default
	\time 6/8 f4(->\fz e8) r4 r8
	f4(->\brack\fz e8) r4 r8
	\dynEO #'(0 . 2) ges4(->\fz f8) r4 r8
% Bars 46 to 50
	ges4.-> r4 r8
	r r ees'16.-> d32-. d8[( c)] ees16.-> d32-.
	d8([ c)] ees16.-> d32-. d8( c) ees(
	c) ees( c) ees( c) ees
	d4->\< d8->~ d << d4->~ {s8 s\!}>>
% Bars 51 to 55
	<< d2. {s4.\< s8 s s\!}>> \mark \default
	ees4.\ff d,16( ees f ees d ees)
	bes'4.~ bes4 ees,16 f
	g8->\< aes-> bes-> c->[ d-> r16 ees-.]\!
	\dynEO #'(0 . 2) ees4.~\fz ees8. d16(\> f ees)\!
% Bars 56 to 60
	d4.(\> << c) {s8 s s\!}>>
	bes4. d,16(\< ees f ees d ees)\!
	c'4.\brack\fz~ c4~ c16 aes
	bes4. d,16(\< ees f ees d ees)\!
	c'4.\brack\fz aes4~ aes16 ees
% Bars 61 to 65
	f4(\< des'16)[ r32 c-.] c4( bes8)\!
	bes4(\< ges'16)[ r32 f-.]\! f4( ees8)
	ees4(\< ges16)[ r32 f-.]\! f4( ees8)
	ees4(\< ges16)[ r32 f-.]\! f4( ees8)
	e4(\< g!16)[ r32 f-.]\! f4( e8)
% Bars 66 to 70
	f4(\< aes16)[ r32 g-.]\! g4( f8)
	\dynEO #'(0 . 2) fis2.->\ff~
	fis~
	fis8 r r r4 r8
	R2.*2
% Bars 71 to 75
	
	r4 r8 r4 ees,16(\brack\p f
	ges4.) f16( ges aes ges f ges)
	ees'4.~ ees4 f,16( ges
	ees'8) f,16(\< ges ees'8) f,16( ges ees'8) a,16([ bes]
% Bars 76 to 80
	ces8) des-.\! d-.\> ees-.[ f-. r16 ges]\! \mark \default
	ges4.(->\pp f)
	ees( des)
	ges->( f)
	ees( des)
% Bars 81 to 85
	aes'(->\< ges)
	f( ees4 ces8)\!
	bes4.(\> aes4 ces8\!
	bes4)\brack\pp r8 r4 r8
	r4 r8 f16(\p\< ges aes ges f ges
% Bars 86 to 90
	ees'4.\> des4)\! r8
	r4 r8 g,!16(\p\< aes bes aes g aes)\!
	ces4.->(-\offset X-offset -0.5 \brack\fz\< << bes) { s8 s s\!}>>
	aes4.(\> ges4 ees8)\!
	des4(\brack\pp ges8 f4 aes8)
% Bars 91 to 95
	ces4.(->_\pocoapococresc ges')
	f( ees4 ces8)\<
	bes4.( aes4)\! r8
	aes'4.( ges4\< ees8)
	des4( ges8) f4( aes8)\! \break
% Bars 96 to 100
	\dynEO #'(0 . 2) ges4.-^\ff r4 r8
	bes,4.-^ r4 r8
	\dynEO #'(0 . 2) ees4.\fz d16( ees f ees d ees)
	\dynEO #'(0 . 2) ges4.\ff~ ges8 f4->
	ees4 d8~ d ees-. f-.
% Bars 101 to 105
	ges4.->~ ges8 f4->
	ees4 d8~ d ees-. f-.
	ges-. r r r4 r8
	ges,8\brack\fz r r b,16(\brack\p c d c b c)
	f8-^-. r r b,16( c d c b c)
% Bars 106 to 110
	f8-^-. r r b,16( c d c b c)
	aes'8-._\fzpocoapococresc r r c,16(\< des ees des c des)\!
	aes'8-.\fz r r c,16(\< des ees des c des)\!
	ces'8-.\fz r r e,16( f g f e f)
	ces'8\brack\fz r r e,16( f g f e f)
% Bars 111 to 115
	c'!8-. e,16( f g f) c'8-.\< e,16( f g f)
	d'8-. a16( bes c bes) f'8-. cis16( d ees d)\!
	aes'!4\ff r8 r4 r8
	R2.*4
	
% Bars 116 to 120
	
	
	ees,,2.\pp~
	ees~
	ees4. d16( ees f ees d ees)
% Bars 121 to 125
	bes'4.->~ bes8 r r
	R2.
	r4 r8 r4 a'16-.\p bes-.
	ces8-.\< c-. des-.\! ees[\>-. f-. r16 ges-.]\!
	ges4 r8 r4 r8
% Bars 126 to 130
	R2.*5
% Bars 131 to 135
	r4 a,16(\p gis a8) b-. c-.\<
	c-. d-. e-.\! e-. fis-.\> g16( fis\!
	g8) r r b,-.\< c-. d~
	d\! c-. b-.\> a-. g-. fis-.\!
	e4\p r8 r4 r8
% Bars 136 to 140
	R2.*3
	
	
	fis'4.(->\mf e~\<
	<< e) {s8 s\! s}>> c4(\> a8)\!
% Bars 141 to 145
	b4.(\< d\>~
	d4 b8 a4 g8)\!
	b2.\p(~
	b4.~ b8 e d)
	c2.~
% Bars 146 to 150
	c4.~ c8\< d-> e->\!
	f!4->\f r8 r4 r8
	R2.*12
% Bars 151 to 155
	
% Bars 156 to 160
	
	
	
	
	\time 3/4 \dynEO #'(0 . 2) a,16(\f bes c bes) a( bes c bes) a( bes c bes)
% Bars 161 to 165
	\textMark "Vi-" \time 6/8 a( bes) c( bes) bes( aes!) aes( g) g( f) f( ees)
	\time 3/4 a16( bes c bes) a( bes c bes) a( bes c bes) \textEndMark "-de"
	\time 6/8 a16( bes) c( bes) bes( aes!) aes( g) g( f) f( ees)
	\dynEO #'(0 . 2) ees'4.\brack\fz d16( ees f ees d ees)
	\dynEO #'(0 . 2) ees4.\brack\fz d16( ees f ees d ees)
% Bars 166 to 170
	\dynEO #'(0 . 2) ges4.\brack\fz f16( ges aes ges f ges)
	\dynEO #'(0 . 2) ges4.\brack\fz f16( ges aes ges f ges) \mark \default
	\dynEO #'(0 . 2) a4\brack\fz r8 r4 r8
	R2.*4
	
% Bars 171 to 175
	
	
	e,2._\fpdim~
	<< e~ {s8\> s s  s s s\!}>>
	e r r r4 r8
% Bars 176 to 180
	c'2.(~\fz\>
	c4. b\!)
	b(\p ais
	gis fis
	e dis
% Bars 181 to 185
	d!4) \once \tupletUp \tuplet 3/2 8 {cis16(\< d e)\!} d4.(~\fz
	<< d4.\> {s8. \markEO #'(0 . 4) s_\dimmarkup}>> cis4.)\!
	fis2.\pp~
	fis4 r8 r4 r8
	R2.
% Bars 186 to 190
	\dynEO #'(0 . 2) d'2.\fz
	\dynEO #'(0 . 2) d4.->(\fz cis)
	\dynEO #'(0 . 2) d2.->\fz
	\dynEO #'(0 . 2) d4.->(\fz cis)
	\dynEO #'(0 . 2) f!2.->\fz
% Bars 191 to 195
	\dynEO #'(0 . 2) f4.(\fz e)
	\dynEO #'(0 . 2) f2.->\fz
	\dynEO #'(0 . 2) f4.->(\fz e)
	\dynEO #'(0 . 2) f4\ff \tuplet 3/2 8 { e16( f g)} f4(-> e8)
	f4 \tuplet 3/2 8 {e16( f g)} f4->( e8)
% Bars 196 to 200
	\acciaccatura g f4(-> e8) f4->( e8)
	f4->( e8) r4 r8
	R2.
	e,8-.\pp e-. r e-. e-. r
	e-. e-. r e-. e-. r
% Bars 201 to 205
	e-. e-. r e-. e-. r
	e-. e-. r e-. e-. r
	<< e'2.~ {\hairpinShorten #'(2.5 . 0) s8-\tweak extra-offset #'(0 . 6.5) \< s s  s s s\!}>>
	<< e2. {\hairpinShorten #'(2.5 . 0) s8-\tweak extra-offset #'(0 . 6.5) \> s s  s s s\!}>>
	f8 r r f,-.\pp f-. r
% Bars 206 to 210
	f-. f-. r f-. f-. r
	f-. f-. r f-. f-. r 
	f-. f-. r f-. f-. r
	<< f'2.~ {s8\< s s  s s s\!}>>
	<< f2. {s8\> s s  s s s\!}>>
% Bars 211 to 215
	\markEO #'(0 . 2) fis2._\ppcrescpocoapoco~
	fis~
	fis
	g(-\alterBroken shorten-pair #'((2 . 0)(4 . 0)) -\alterBroken extra-offset #'((0 . 6.3)()) \<
	<< aes!) {s8 s s  s s s\!}>> \mark \default
% Bars 216 to 220
	\dynEO #'(0 . 3) g4.\ff-^ fis-^
	e-^ d-^
	g-^ fis-^
	e-^ d-^
	\hairpinShorten #'(2 . 3) a'4(->\< g8) f4->(\! e8)
% Bars 221 to 225
	\hairpinShorten #'(3 . 2) b'4(->\< a8) g4->(\! fis8)
	\hairpinShorten #'(3 . 2) c'4(->\< b8) a4->(\! gis8)
	\hairpinShorten #'(3 . 0) a4(->\< gis8)\! a4->( gis8)
	\dynEO #'(0 . 2) bes!2.\fff
	bes8 r r d, r r
% Bars 226 to 230
	\time 3/4 d8-. r d-. r d-. r
	\time 6/8 d r r r4 r8
	R2.
	\time 3/8 R4.
	\time 6/8 R2.*25
% Bars 231 to 235
	
% Bars 236 to 240
	
% Bars 241 to 245
	
% Bars 246 to 250
	
% Bars 251 to 255
	
	
	
	\mark \default
	e2.~\fp\>
% Bars 256 to 260
	<< e {s4. s\!}>>
	R2.*6
% Bars 261 to 265
	
	
	fis,4.(~\p fis16 eis gis fis eis fis)
	cis'4.~ cis4 fis,16-. gis-.
	a8-.\< b-. cis-. d!-. e-. fis-.\!
% Bars 266 to 270
	fis4.\fp~ fis8. eis16( gis fis)
	fis4.-!\> << e-! {s8 s s\!}>>
	d4.\pp~ d8. cis16( e d)
	d4.-! a-!
	d~ d8. cis16-. e-. d-.
% Bars 271 to 275
	d4.-> c!->
	b4.~ b8. ais16-. c-. b-.
	b4.-! a-!
	g~ g8. fis16-. a-. g-.
	g4.\<_\pocoapococresc << e {s8 s s\!}>>
% Bars 276 to 280
	g4.~ g8. fis16-. a-. g-.
	g4. e
	bes'~ bes8. a16-. c-. bes-.
	bes4.-> g->
	bes4.~ bes8. a16-. c-. bes-.
% Bars 281 to 285
	bes4.-^ g-^
	<< ees'2. {s8\< s s  s s s\!}>>
	\dynEO #'(0 . 2) ees8\ff r ges16.-. f32-. f4( ees8)
	r4 c16.-. bes32-. bes4( aes8)
	r4 f'16.-. ees32-. ees4( d8)
% Bars 286 to 290
	r4 b16.-. a32-. a4( g8)
	bes!4( g16) r cis4( bes16) r
	cis4( bes8) r4 r8
	\dynEO #'(0 . 2) a2.\fz~
	a
% Bars 291 to 295
	\dynEO #'(0 . 2) bes!\fz~
	bes
	\dynEO #'(0 . 2) c\fz~
	c
	\dynEO #'(0 . 2) des\fz-\tweak extra-offset #'(0 . 0.5) \<~
% Bars 296 to 300
	<< \markEO #'(0 . 3.5) des_\pocoapococresc {s4 s s\!}>>
	d!2.->
	ees->
	e->
	fis->
% Bars 301 to 305
	\dynEO #'(0 . 2) aes\ff~
	<< aes {s8\< s s  s s s\!}>>
	r aes( aes,) a'( a,) r
	r bes'( bes,) b-^ c-^ d-^ 
	\dynEO #'(0 . 2) ees2.\ff~
% Bars 306 to 310
	ees~
	ees
	\dynEO #'(0 . 2) fis-^\ff~
	fis
	ges8-^ r r r4 r8
% Bars 311 to 315
	ces,8-.-^ r r r4 r8
	des-.-^ r r r4 r8
	bes-. r r ces-. r r
	c!-. r r des-. r r
	d!2._\pcresc
% Bars 316 to 320
	f4.-^ \dynEO #'(0 . 2) bes8-.\f r r
	\dynEO #'(0 . 2) ees,4.-^\ff r4 r8
	bes4.-^ des-^
	des4-^ bes16-. c-. d8-.-^ ees-.-^ f-.-^
	g2.-^
% Bars 321 to 325
	ges4. f8( ges aes)
	bes4.~ bes8 r r
	ees,-^ r r f-^ r r
	ees-^ r r f-^ r r
	f-^ r r f-^ r r
% Bars 326 to 330
	f-^ r r f-^ r r
	des-.\p des-. r des-. des-. r
	des-. des-. r \dynEO #'(0 . 2) d-^\f d-^ d-^
	des2.\p~
	des4. \dynEO #'(0 . 2) d8\f-^ d-^ d-^
% Bars 331 to 335
	g2.\p~
	g_\brackpocoapococresc~
	g~
	g~
	<< g {s8\< s s  s s s\!}>> \break
% Bars 336 to 340
	\dynEO #'(0 . 2) g2.-^\f~
	g4 aes8~ aes aes-^ aes-^
	\dynEO #'(0 . 2) g2.\ff~
	g4 r8 r4 r8
	g4 r8 r4 r8
% Bars 341 to 343
	g4 r8 r4 r8
	g4 r8 r4 r8
	\dynEO #'(0 . 2) ees2.\ff\fermata \fine
}
