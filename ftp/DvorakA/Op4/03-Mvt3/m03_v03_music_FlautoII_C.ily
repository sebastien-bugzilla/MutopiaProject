%  work        : Symphony No. 2 in B-flat Major Op.4
%  typesetter  : Sébastien MANEN
%  date        : Friday 31 January 2025, 23:08
%###############################################################################
%#                          M U S I C   S E C T I O N                          #
%###############################################################################
musicFlautoIIMvtIII = \relative c {
	\clef treble
	\key bes \major
%	\transposition a
% Bars 1 to 5
	R2.*4
	
	
	
	a'''2.\pp\<~
% Bars 6 to 10
	a~
	<< a~ {s4 s s\!}>>
	a2.\f
	R2.*4
	
% Bars 11 to 15
	
	
	d2.\pp\<~
	<< d~ {s4 s s\!}>>
	d2.~
% Bars 16 to 20
	d\f
	R2.*4
	
	
	
% Bars 21 to 25
	g,2.\pp\<~
	g~
	<< g~ {s4 s s\!}>>
	g2.\f <>^\mutainflpicc \changeStaffName #flpicc
	R2.*6
% Bars 26 to 30
	
% Bars 31 to 35
	r4 r \dynEO #'(0 . 1) g16(\mf f ees g
	f8) r r4 g16(\< f ees g
	f8)\! r r4 f16(\f g f g
	f8) r r4 f16( g f g
	f8) r r4 f16( ees d bes)
% Bars 36 to 40
	\dynEO #'(0 . 2) f'2.\ff\startTrillSpan
	\dynEO #'(0 . 1) f\fz\startTrillSpan
	\dynEO #'(0 . 1) f\fz\startTrillSpan
	\dynEO #'(0 . 1) f\fz\startTrillSpan
	\dynEO #'(0 . 1) f\fz\startTrillSpan
% Bars 41 to 45
	\afterGrace 11/12 f\fz\startTrillSpan {e16 f\stopTrillSpan}
	f,8 r^\mutainflii r4 r \changeStaffName #II
	R2.
	R2.\fermata \mark \default
	R2.*18
% Bars 46 to 50
	
% Bars 51 to 55
	
% Bars 56 to 60
	
% Bars 61 to 65
	
	
	f2.\pp~
	f~
	f~
% Bars 66 to 70
	f\fermata
	bes2._\piuf
	ees4( f aes)
	g4( f\< a!\!
	bes8)[-. r16 bes-.] bes2~
% Bars 71 to 75
	<< bes2. {s4\< s s\!}>>
	\dynEO #'(0 . 1) ees4(\f\> d bes)\!
	bes(\> a ees8. ees16-.)\!
	ees2->(~_\dimmarkup ees8. c16-.)
	<< bes2. {s4\< s s\!}>>
% Bars 76 to 80
	f'4( g aes)
	g( << bes2) {s4\< s\!}>>
	d8.\f d16 d2
	<< des2(\p {s4 s\>} >> ges,)
	f( des' c)\p
% Bars 81 to 85
	bes( aes\> g)\!
	g2(\pp aes4)
	g2( aes4)
	g2 g8. g16 \mark \default
	f8 r r4 r
% Bars 86 to 90
	R2.*11
% Bars 91 to 95
	
% Bars 96 to 100
	
	d'4(\f c bes)
	f'->( ees d)
	a'->( g f)
	bes->( a g)
% Bars 101 to 105
	f8.[\ff ees16 c8. g'16] f4~
	f8.[ ees16 c8. g'16] f4~
	f8.[ ees16 c8. g'16] f4~
	f8.[ ees16 c8. g'16] f4~
	f8.[ g16 ees8. c16] f4~
% Bars 106 to 110
	f8.[ g16 ees8. c16] f4~
	f8.[ g16 ees8. c16] f4~
	f8.[ g16 ees8. c16] f4
	f2.\ff~
	f
% Bars 111 to 115
	R2. \mark \default
	R2.*2
	
	\dynEO #'(0 . 1) ees8.\f ees16 ees8 r r4
	d8. d16 d8 r r4
% Bars 116 to 120
	c8. c16 c8 r r4
	bes8. bes16 bes8 r r4
	g->-. bes-.-> bes-.->
	c8. c16 c8 r r4
	g4-.-> bes-.-> bes-.->
% Bars 121 to 125
	c8. c16 c8 r r4
	d8. d16 d8 r r4
	d8. d16 d8 r r4
	cis8. cis16 cis8 r r4
	a8. a16 a8 r r4
% Bars 126 to 130
	g4-.-> a-.-> bes-.->
	c8. c16 c8 r r4
	g-.-> a-.-> bes-.->
	c8. c16 c8 r a8. bes16
	bes2-> a8. bes16
% Bars 131 to 135
	bes2-> a8. bes16
	bes4-.-> a-.-> bes-.->
	a-.-> bes-.-> a-.-> \mark \default
	R2.*2
	
% Bars 136 to 140
	\dynEO #'(-0.5 . 0) bes8-.\ff bes-. r4 r
	R2.
	g8-.\f ees'-. r4 r
	R2.
	g,8-.\f ees'-. r4 r
% Bars 141 to 145
	R2.*3
	
	
	fis,4 g a
	bes a bes
% Bars 146 to 150
	a2.\fermata
	a16( bes f8-.) f4. f8
	a16( bes f8-.) f4. f8
	a16( bes f8-.) f4. f8
	a16( bes f8-.) f4. f8 \mark \default
% Bars 151 to 155
	R2.*4
	
	
	
	a2.\pp~
% Bars 156 to 160
	a\<~
	<< a~ {s4 s s\!}>>
	a2.\f
	R2.*4
	
% Bars 161 to 165
	
	
	d2.\pp~
	d\<
	<< d~ {s4 s s\!}>>
% Bars 166 to 170
	d2.\f
	R2.*2
	
	g,2.\p\<~
	g\!
% Bars 171 to 175
	R2.
	<< g2. {s4\p\< s s\!}>>
	R2.
	<< g2. {s4\p\< s s\!}>>
	R2.*4
% Bars 176 to 180
	
	
	\section
	\repeat volta 2 {
		\key a \major R2.*8
% Bars 181 to 185
	
% Bars 186 to 190
		
		cis2.\pp
		cis8. a16 a8 r r4
		cis2.
		cis8. a16 a8 r r4
% Bars 191 to 195
		a2.\pp
		a8. fis16 fis8 r r4
		a2.
		a8. fis16 fis8 r r4
		R2.*4
% Bars 196 to 200
		
		
		
		fis'2.\pp
		fis8. d16 d8 r r4
% Bars 201 to 205
		fis2.
		fis8. d16 d8 r r4
		d2.
		d8. b16 b8 r r4
		d2.
% Bars 206 to 210
		d8. b16 b8 r r4
		\tuplet 3/2 4 {g8(\pp a b)} fis8-. fis-. g4->
		\tuplet 3/2 4 {\hairpinShorten #'(0 . -3) g8(\crescD\< g b)} fis8-. fis-. g4->\!
		\tuplet 3/2 4 {a8(\< b c)} gis-. gis-. a4->\!
		\tuplet 3/2 4 {a8( b c)} gis-. gis-. a4
% Bars 211 to 215
		f4.\f g8-. a16[( bes) r16. e,32]
		e4( f) d8-. d-.
		e4 c2->
		d bes8. c16
		c4-> d-> e->
% Bars 216 to 220
		d2 bes8. c16
		c4-> d-> e->
		bes' r r \mark \default
	}
	a4.(\p b8) \tuplet 3/2 4 {cis( d gis,)}
	gis4( a fis8-. fis-.)
% Bars 221 to 225
	gis4 e2->\fz
	r4 r gis16(\pp a b cis)
	r4 r gis16( a b cis)
	r4 r ais16( b cis d)
	r4 r ais16( b cis d)
% Bars 226 to 230
	R2.
	R2.*3
	
	
	R2.*5
% Bars 231 to 235
	
	
	
	
	\dynEO #'(0 . 2) ees16(\f f g8) c,-. c-. d-. d-.
% Bars 236 to 240
	bes-.\> aes-. aes-. g-. f-. f-.\!
	ees\p-. r r4 r
	R2.
	fis16(\fz gis! ais8)\> dis,-. dis-. cis-. cis-.\!
	b\p-. b-. ais-. ais-. b-. gis-. 
% Bars 241 to 245
	ais r r4 r
	R2.
	a'!16(\p b cis8) gis-. gis-. a4->
	a16( b cis8) gis-. gis-. a4->
	b16( cis d8) ais-. ais-. b4->
% Bars 246 to 250
	b16( cis d8) ais-. ais-. b4-> \mark \default
	d2.\pp~
	d2 d8. d16
	d2.~
	d2 d8. d16
% Bars 251 to 255
	d2.~
	d2 d8. d16
	d2.~
	d \section
	\key bes \major r8 bes,16(\p c \tuplet 3/2 4 {d8\< ees f g aes bes\!
% Bars 256 to 260
	c\> bes aes g f ees\! d c bes)}
	r8 bes16( c \tuplet 3/2 4 {d8\< ees f g aes bes\!
	c\> bes aes g f ees\! d c bes)}
	r8 bes16( c \tuplet 3/2 4 {d8\< ees f g aes bes\!
	c\> bes aes g\! f ees d c bes)}
% Bars 261 to 265
	r8 bes16( c \tuplet 3/2 4 {d8 ees f g\< aes bes\!
	c\> bes aes g\! f ees d f aes)
	r aes,( bes \once \tupletUp ces\< des ees f aes ces)\!
	des(\fz ces bes\> aes ges f ees des ces)\!
	r aes( bes ces\< des ees f aes ces)\!
% Bars 266 to 270
	des(\f ces bes aes ges f ees! des ces)}
	f2.\ff\<\startTrillSpan
	<< f\startTrillSpan {s4 s s8 s\!\stopTrillSpan}>> \mark \default
	bes4-. r r
	R2.*3
% Bars 271 to 275
	
	
	a2.\pp\<~
	a
	a~
% Bars 276 to 280
	<< a {s4 s s\!}>>
	R2.*4
% Bars 281 to 285
	d2.\pp\<~
	d~
	d~
	<< d {s4 s s\!}>>
	R2.*4
% Bars 286 to 290
	
	
	
	g,2.\pp\<~
	g~
% Bars 291 to 295
	g~
	<< g {s4 s s\!}>> \changeStaffName #flpicc
	R2.*6^\mutainflautopiccolo 
	
	
% Bars 296 to 300
	
	
	
	r4 r g16(\mf f ees bes'
	f8) r r4 g16( f ees bes'
% Bars 301 to 305
	f8) r r4 \acciaccatura e'8\f f r
	r4 r \acciaccatura e8\f f r
	r4 r \acciaccatura e8\f f r
	f2.\startTrillSpan
	f\startTrillSpan
% Bars 306 to 310
	f\startTrillSpan
	f\startTrillSpan
	<< f\startTrillSpan {s4\> s s8 s\!}>>
	<< f2.\p\startTrillSpan {s4 s s8 s\stopTrillSpan}>>
	f,,8 r r4^\mutainflautoii r \changeStaffName #II
% Bars 311 to 315
	R2.
	R\fermata \mark \markAaBox
	R2.*18
% Bars 316 to 320
	
% Bars 321 to 325
	
% Bars 326 to 330
	
% Bars 331 to 335
	f2.\pp~
	f~
	f~
	f\fermata
	bes_\piuf
% Bars 336 to 340
	ees4( f aes)
	g4( f\< a!\!
	bes8-.)[ r16 bes-.] bes2->~
	<< bes2. {s4\< s s\!}>>
	ees(\f\> d bes)\!
% Bars 341 to 345
	bes(\> a ees8. ees16-.)\!
	ees2->(~_\dimmarkup ees8. c16-.)
	<< bes2. {s4\< s s\!}>>
	f'4( g aes)
	g( << bes2) {s4\< s\!}>>
% Bars 346 to 350
	d8.\f d16 d2
	<< des2(\p {s4 s\>}>> ges,4)
	f( des' c)\p
	bes( aes\> g)\!
	g2(\pp aes4)
% Bars 351 to 355
	g2( aes4)
	g2 g8. g16 \mark \markBbBox
	f8 r r4 r
	R2.*11
% Bars 356 to 360
	
% Bars 361 to 365
	
	
	
	
	d'4(\f c bes)
% Bars 366 to 370
	f'(-> ees d)
	a'(-> g f)
	bes(-> a g)
	f8.[\ff ees16 c8. g'16] f4~
	f8.[ ees16 c8. g'16] f4~
% Bars 371 to 375
	f8.[ ees16 c8. g'16] f4~
	f8.[ ees16 c8. g'16] f4~
	f8.[ g16 ees8. c16] f4~ 
	f8.[ g16 ees8. c16] f4~ 
	f8.[ g16 ees8. c16] f4~ 
% Bars 376 to 380
	f8.[ g16 ees8. c16] f4
	f2.\ff~
	f
	R2. \mark \markCcBox
	R2.*2
% Bars 381 to 385
	
	\dynEO #'(0 . 1) ees8.\f ees16 ees8 r r4
	d8. d16 d8 r r4
	c8. c16 c8 r r4
	bes8. bes16 bes8 r r4
% Bars 386 to 390
	g4-.-> bes-.-> bes-.->
	c8. c16 c8 r r4
	g4-.-> bes-.-> bes-.->
	c8. c16 c8 r r4
	d8. d16 d8 r r4
% Bars 391 to 395
	d8. d16 d8 r r4
	cis8. cis16 cis8 r r4
	a8. a16 a8 r r4
	g4-.-> a-.-> bes-.->
	c8. c16 c8 r r4
% Bars 396 to 400
	g-.-> a-.-> bes-.->
	c8. c16 c8 r a8. bes16
	bes2-> a8. bes16
	bes2-> a8. bes16
	bes4-.-> a-.-> bes-.->
% Bars 401 to 405
	a-.-> bes-.-> a-.-> \mark \markDdBox
	R2.*2
	
	\dynEO #'(0 . 1) bes8-.\ff bes-. r4 r
	R2.
% Bars 406 to 410
	g8-.\f ees'-. r4 r
	R2.
	g,8-.\f ees'-. r4 r
	R2.*3
	
% Bars 411 to 415
	
	fis,4\brack\ff g a
	bes a bes
	a2.\fermata
	bes4.(\p c8 d16[ ees r16. a,32-.)]
% Bars 416 to 420
	a4(-> bes-> g8-. g-.)
	a4-> f2->
	ees4(_\piup f g)
	d8. d16 d2->
	g4( f\> ees
% Bars 421 to 425
	a8[ r16 a] a2\!)
	bes4.(\pp c8 d16[ ees r16. a,32-.])
	a4. r8 r4
	bes4.( c8 d16[ ees r16. a,32-.])
	a4. r8 r4
% Bars 426 to 430
	R2.*8
% Bars 431 to 435
	
	
	
	bes2(\pp\< ees4)
	d( bes\! g8.\> g16)
% Bars 436 to 440
	f2_\dimmarkup f4\!
	f(\pp c'4.\fermata d8
	bes8) r r4 r
	R2.*7
% Bars 441 to 445
	
% Bars 446 to 450
	d,8\mf d ees ees a a
	f f ees ees  ees ees
	d\< d f f bes bes
	a a a a a a\!
	bes\f bes a a bes bes
% Bars 451 to 455
	cis cis cis cis cis cis
	d d c! c b b
	c c cis cis  cis cis
	d d c! c b b
	c\ff c c c bes! bes
% Bars 456 to 460
	bes8. bes16 bes2
	des8. des16 des2
	bes8. bes16 bes2
	des8. des16 des2
	e2.-^~
% Bars 461 to 465
	e
	ees!4->-. ees-.-> ees-.->
	a,-.-> bes-.-> c-.->
	\dynEO #'(0 . 2) bes4.\ff bes8 bes4
	bes4. bes8 bes4
% Bars 466 to 470
	a4. a8 a4
	d4. d8 d4
	g,4. g8 g4
	g4. g8 g4
	bes4. bes8 bes4
% Bars 471 to 475
	d4. d8 d4
	c4. c8 c4
	cis4. cis8 cis4
	d2.~
	d~
% Bars 476 to 480
	d~
	d
	c(
	bes)
	c4-. b-. c-.
% Bars 481 to 485
	b-. c-. d-.
	c-. c-. c-.
	c-. c-. c-.
	b-. r r
	R2.*2
% Bars 486 to 490
	
	c4.\f c8 c4
	f, r r
	f-. f-.\> f-._\dimmarkup
	f-.\! r r
% Bars 491 to 495
	f,-.\p f-. f-.
	r r f-._\pdim
	r r f-.
	r r f-.
	r r f-.
% Bars 496 to 500
	R2.
	ees'\pp
	R
	d\pp
	R
% Bar 501
	bes'\fermata\pp \fine
}
