%  work        : Symphony No. 2 in B-flat Major Op.4
%  typesetter  : Sébastien MANEN
%  date        : Friday 31 January 2025, 23:08
%###############################################################################
%#                          M U S I C   S E C T I O N                          #
%###############################################################################
musicOboeIMvtIII = \relative c {
	\clef treble
	\key bes \major
%	\transposition a
% Bars 1 to 5
	R2.*4
	
	
	
	f''2.\pp\<~
% Bars 6 to 10
	f
	<< d~ {s4 s s\!}>>
	d2.\f
	R2.*4
	
% Bars 11 to 15
	
	
	d2.\pp\<~
	<< d {s4 s s\!}>>
	d2.~
% Bars 16 to 20
	d\f
	R2.*4
	
	
	
% Bars 21 to 25
	ees2.\pp\<~
	ees
	<< e~ {s4 s s\!}>>
	e2.\f
	r4 r r8 g,(_\pplegato
% Bars 26 to 30
	a4 bes d)
	c2~ c8 e(
	f4 bes a)
	f2~ f8 g(
	a4 bes a
% Bars 31 to 35
	c8) r f,4(->\f ees!->\<~
	ees) \markEO #'(0 . 1.8) f(_\fzmarkup ees)\!
	f8(->\f e ees d des c)
	f(-> e ees d des c)
	f2.
% Bars 36 to 40
	c'4->\f( bes a)
	bes( a g)
	a( g f)
	ees( d c
	bes a g)
% Bars 41 to 45
	f2.\fz~
	<< f~ {s4\> s s8 s\!}>>
	f2.\p\>~
	f\fermata\! \mark \default
	R2.*12
% Bars 46 to 50
	
% Bars 51 to 55
	
% Bars 56 to 60
	
	\mmrLength #15 R2.*3
	
	
	\ni \mmrPos #-4 R2.
% Bars 61 to 65
	\mmrPos #-4 R \no
	\voiceTwo r4 r \oneVoice \tuplet 3/2 4 {e'8(\p f g)}
	a2( g8 f
	a2) g8( f
	a2) g8( f
% Bars 66 to 70
	a2.)\fermata
	bes,2(_\piuf bes4
	bes2 d4)
	ees( f\< ees\!
	d ees f)
% Bars 71 to 75
	g(\< f ees\!~
	ees\f d bes)\>
	ees2~ ees8. ees16\!
	ees2._\dimmarkup
	bes2\< bes4\!
% Bars 76 to 80
	f'( g aes)
	g( f\< ees)\!
	a!2(\f bes4)
	aes2(\p\> ges4)
	f( g! aes)\p
% Bars 81 to 85
	f2.\>
	g2(\pp aes4)
	g2( aes4)
	g2 e8. e16 \mark \default
	ees!4.\fz ees8-. ees-. c-.
% Bars 86 to 90
	c4(\> d)\! r
	ees4.\fz ees8-. ees-. c-.
	c4( d) r
	R2.*4
% Bars 91 to 95
	
	
	\ni \mmrPos #-4 R2.
	\mmrPos #-4 R
	\mmrPos #-4 R
% Bars 96 to 100
	\mmrPos #-4 R \no
	d4(\f c bes)
	f'( ees d)
	a'( g f)
	bes(-> a g)
% Bars 101 to 105
	f8.[\ff ees16 c8. g'16] f4~
	f8.[ ees16 c8. g'16] f4~
	f8.[ ees16 c8. g'16] f4~
	f8.[ ees16 c8. g'16] f4~
	f8.[ g16 ees8. c16] f4~
% Bars 106 to 110
	f8.[ g16 ees8. c16] f4~
	f8.[ g16 ees8. c16] f4~
	f8.[ g16 ees8. c16] f4
	f2.\ff~
	f
% Bars 111 to 115
	R2. \mark \default
	ees4.\f ees8-. ees-. c-.
	c4( d bes)
	bes'8.\f bes16 bes8 r r4
	bes8. bes16 bes8 r r4
% Bars 116 to 120
	a8. a16 a8 r r4
	f8. f16 f8 r r4
	ees-.-> f-.-> bes-.->
	a8. a16 a8 r r4
	ees-.-> f-.-> bes-.->
% Bars 121 to 125
	a8. a16 a8 r r4
	bes8. bes16 bes8 r r4
	a8. a16 a8 r r4
	g8. g16 g8 r r4
	f8. f16 f8 r r4
% Bars 126 to 130
	g-.-> a-.-> bes-.->
	e,8. e16 e8 r r4
	g-.-> a-.-> bes-.->
	e,8. e16 e8 r a8. bes16
	bes2-> a8. bes16
% Bars 131 to 135
	bes2-> a8. bes16
	bes4-.-> a-.-> bes-.->
	a-.-> bes-.-> a-.-> \mark \default
	R2.*2
	
% Bars 136 to 140
	f8-.\ff bes,-. r4 r
	R2.
	\tuplet 3/2 4 {ees,8-.\ff\< g-. bes-.\!} ees2->
	R2.
	\tuplet 3/2 4 {ees,8-.\f\< g-. bes-.} ees2\ff
% Bars 141 to 145
	ees4-.\ff f-. g-.
	ees-. f-. g-.
	ees-. f-. g-.
	fis g a
	bes a bes
% Bars 146 to 150
	a2.\fermata
	c16( d bes8-.) bes4. bes8
	c16( d bes8-.) bes4. bes8
	c16( d bes8-.) bes4. bes8
	c16( d bes8-.) bes4. bes8 \mark \default
% Bars 151 to 155
	R2.*4
	
	
	
	f2.\pp~
% Bars 156 to 160
	f\<~
	<< f~ {s4 s s\!}>>
	f2.\f
	R2.*4
	
% Bars 161 to 165
	
	
	bes2.\pp~
	bes\<
	<< b~ {s4 s s\!}>>
% Bars 166 to 170
	b2.\f
	R2.*2
	
	ees,2.(\p\<
	e)\!
% Bars 171 to 175
	R2.
	ees!2(\p\< e4)\!
	R2.
	ees!2(\p\< e4)\!
	R2.*4
% Bars 176 to 180
	
	
	\section
	\repeat volta 2 {
		\key a \major R2.*14
% Bars 181 to 185
	
% Bars 186 to 190
	
% Bars 191 to 195
		
		
		\ni \mmrPos #-4 R2.
		\mmrPos #-4 R \no
		R2.
% Bars 196 to 200
		a2.\pp~
		a~
		a
		R2.
		a~
% Bars 201 to 205
		a~
		a
		fis,\pp\<~
		fis4 b8( cis d b)\!
		fis'2.\f\>~
% Bars 206 to 210
		fis4\! r r
		d2\pp(~ d8 fis
		d2\<_\crescmarkup~ d8 fis)\!
		c2(~\< c8 e\!
		c2~ c8 e)
% Bars 211 to 215
		a4.\f bes8-. cis16[( d) r16. g,32]
		g4( a) bes8-. bes-.
		g4 a2->
		f2 g8. a16
		a4-> bes-> c->
% Bars 216 to 220
		f,2 g8. a16
		a4-> bes-> c->
		f, r r \mark \default
	}
	R2.*8
	
% Bars 221 to 225
	
% Bars 226 to 230
	
	e4.(\mp^\solo f8) \tuplet 3/2 4 {g( a d,)}
	d4( e) f8-. f-.
	d4 e2->
	R2.*5
% Bars 231 to 235
	
	
	
	
	g16(\f aes bes8) g-. g-. bes-. bes-.
% Bars 236 to 240
	ees,-.\> ees-. ees-. ees-. ees-. d-.\!
	ees-.\p r r4 r
	R2.*7
% Bars 241 to 245
	
	
	
	
	\ni \mmrPos #-4 R2.
% Bars 246 to 250
	\mmrPos #-4 R \no \mark \default
	f2.\pp~
	f2 f8. f16
	f2.~
	f2 f8. f16
% Bars 251 to 255
	f2.~
	f2 f8. f16
	f2.~
	f2 f8. f16 \section
	\key bes \major f4 r r
% Bars 256 to 260
	R2.*9
% Bars 261 to 265
	
	
	
	
	\ni \mmrPos #-4 R2.
% Bars 266 to 270
	\mmrPos #-4 R \no
	\trillSpanPadding #-1 f2.\ff\startTrillSpan
	<< f\startTrillSpan {s4 s s8 s\stopTrillSpan}>> \mark \default
	bes4-. r r
	R2.*3
% Bars 271 to 275
	
	
	f2.\pp\<~
	f~
	f~
% Bars 276 to 280
	<< f {s4 s s\!}>>
	R2.*4
	
	
	
% Bars 281 to 285
	bes2.\pp\<~
	bes
	b~
	<< b {s4 s s\!}>>
	R2.*4
% Bars 286 to 290
	
	
	
	ees,2.\pp\<~
	ees
% Bars 291 to 295
	e~
	<< e {s4 s s\!}>>
	r4 r r8 g,\p
	a4( bes d)
	c2~ c8 e
% Bars 296 to 300
	f4( bes a)
	f2(~ f8 g
	a4 bes d
	c8) r r4 r
	R2.*4
% Bars 301 to 305
	
	
	
	c4(\f bes a)
	bes( a g)
% Bars 306 to 310
	a( g f)
	ees( d c)
	\once \stemUp bes(\> a g)\!
	\dynEO #'(0 . -7) f2.^\p\>~
	<< f {s4 s\! s}>>
% Bars 311 to 315
	f2.\>~
	f\pp\fermata \mark \markAaBox
	R2.*15
% Bars 316 to 320
	
% Bars 321 to 325
	
% Bars 326 to 330
	
	
	\ni \mmrPos #-4 R2.
	\mmrPos #-4 R \no
	r4 r \tuplet 3/2 4 {e'8(\p f g)}
% Bars 331 to 335
	\shape #'((0 . 0)(0 . -0.3)(0 . -0.3)(0 . 0)) Slur a2( g8 f
	a2) g8( f
	a2) g8( f
	a2.)\fermata
	bes,2(_\piuf bes4
% Bars 336 to 340
	bes2 d4)
	ees( f\< ees\!
	d ees f)
	g(\< f ees~\!
	ees\f d \hairpinShorten #'(1 . 0) bes)\>
% Bars 341 to 345
	ees2~ ees8. ees16\!
	ees2._\dimmarkup
	\stemUp bes2\< bes4\! \stemNeutral
	f'( g aes)
	g( f\< ees\!)
% Bars 346 to 350
	a!2(\f bes4)
	<< aes2(\p {s4 s\> }>> ges4)
	f( g! aes)\p
	f2.\>
	g2(\pp aes4)
% Bars 351 to 355
	g2( aes4)
	g2 e8. e16 \mark \markBbBox
	ees!4.\fz ees8-. ees-. c-.
	c4(\> d)\! r
	ees4.\fz ees8-. ees-. c-.
% Bars 356 to 360
	c4( d) r
	R2.*8
% Bars 361 to 365
	
	
	
	
	d4(\f c bes)
% Bars 366 to 370
	f'( ees d)
	a'( g f)
	bes(-> a g)
	f8.[\ff ees16 c8. g'16] f4~
	f8.[ ees16 c8. g'16] f4~
% Bars 371 to 375
	f8.[ ees16 c8. g'16] f4~
	f8.[ ees16 c8. g'16] f4~
	f8.[ g16 ees8. c16] f4~
	f8.[ g16 ees8. c16] f4~
	f8.[ g16 ees8. c16] f4~
% Bars 376 to 380
	f8.[ g16 ees8. c16] f4
	f2.\ff~
	f
	R2. \mark \markCcBox
	ees4.\f ees8-. ees-. c-.
% Bars 381 to 385
	c4( d bes)
	bes'8.\f bes16 bes8 r r4
	bes8. bes16 bes8 r r4
	a8. a16 a8 r r4
	f8. f16 f8 r r4
% Bars 386 to 390
	ees-.-> f-.-> bes-.->
	a8. a16 a8 r r4
	ees-.-> f-.-> bes-.->
	a8. a16 a8 r r4
	bes8. bes16 bes8 r r4
% Bars 391 to 395
	a8. a16 a8 r r4
	g8. g16 g8 r r4
	f8. f16 f8 r r4
	g-.-> a-.-> bes-.->
	e,8. e16 e8 r r4
% Bars 396 to 400
	g-.-> a-.-> bes-.->
	e,8. e16 e8 r a8. bes16
	bes2-> a8. bes16
	bes2-> a8. bes16
	bes4-.-> a-.-> bes-.->
% Bars 401 to 405
	a-.-> bes-.-> a-.-> \mark \markDdBox
	R2.*2
	
	f8-.\ff bes,-. r4 r
	R2.
% Bars 406 to 410
	\tuplet 3/2 4 {ees,8-.\ff\< g-. bes-.\!} ees2->
	R2.
	\tuplet 3/2 4 {ees,8-.\f\< g-. bes-.\!} ees2\ff
	ees4-.\ff f-. g-.
	ees-. f-. g-.
% Bars 411 to 415
	ees-. f-. g-.
	fis g a
	bes a bes
	a2.\fermata
	\textMark "Coda" R2.*5
% Bars 416 to 420
	
	
	
	
	\ni \mmrPos #-4 R2.
% Bars 421 to 425
	\mmrPos #-4 R
	\mmrPos #-4 R \no
	a,4.(\pp \once \stemUp bes8 c16[ d r16. f,32-.])
	f4. r8 r4
	a4.( \once \stemUp bes8 c16[ d r16. f,32-.)]
% Bars 426 to 430
	f4 r r
	R2.*3
	
	
	R2.
% Bars 431 to 435
	\mmrnDown R2.*3
	
	
	d'4(\< ees g
	f d\! bes8.\> bes16)
% Bars 436 to 440
	bes2( c4)\!
	c(\pp ees4.\fermata d8
	d) r r4 r
	R2.*3
	
% Bars 441 to 445
	
	f,8\p f g[ g] a a
	bes bes g[ g] f f
	f\< f f f bes[ bes]
	d d d d  c[ c]\!
% Bars 446 to 450
	bes\mf bes bes bes ees[ ees]
	f f ees[ ees] f f
	f\< f f f g[ g]
	ees ees ees ees ees ees\!
	f\f f f f f f
% Bars 451 to 455
	a a a a a a
	aes aes g[ g] f f
	g g g g g g 
	aes aes g[ g] f! f
	g\ff g g g  f[ f]
% Bars 456 to 460
	ees8. ees16 ees2
	aes8. aes16 aes2
	ees8. ees16 ees2
	aes8. aes16 aes2
	bes2.-^~
% Bars 461 to 465
	bes
	bes4-.-> a-.-> bes-.->
	a-.-> g-.-> a-.->
	bes4.\ff bes8 bes4
	g4. g8 g4
% Bars 466 to 470
	a4. a8 a4
	a4. a8 a4
	g4. g8 g4
	g4. g8 g4
	f4. f8 f4
% Bars 471 to 475
	b4. b8 b4
	g4. g8 g4
	a4. a8 a4
	f2.~
	f~
% Bars 476 to 480
	f~
	f
	a(
	bes)
	g4-. aes-. g-.
% Bars 481 to 485
	aes-. g-. aes-.
	g-. f-. g-.
	f-. g-. f-.
	g-. r r
	R2.*2
% Bars 486 to 490
	
	g4.\f g8 g4
	ees! r r
	f-. ees\>-. d-._\dimmarkup
	ees-.\! r r
% Bars 491 to 495
	R2.*6
% Bars 496 to 500
	
	ees2.\pp
	R
	d\pp
	R
% Bar 501
	f\pp\fermata \fine
}
