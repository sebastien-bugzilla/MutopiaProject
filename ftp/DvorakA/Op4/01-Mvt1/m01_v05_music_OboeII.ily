%  work        : Symphony No. 2 in B-flat Major Op.4
%  typesetter  : Sébastien MANEN
%  date        : Friday 31 January 2025, 23:08
%###############################################################################
%#                          M U S I C   S E C T I O N                          #
%###############################################################################
musicOboeIIMvtI = \relative c {
	\clef treble
	\key bes \major
%	\transposition a
% Bars 1 to 5
	\startMeasureCount d'2\pp~
	d~
	d~
	d~
	d~
% Bars 6 to 10
	d~
	d~
	d~
	d~\crescD\<
	d~
% Bars 11 to 15
	d~
	d\! \stopMeasureCount
	e\fermata\ff
	\startMeasureCount fis\pp~
	fis~
% Bars 16 to 20
	fis~
	fis~
	fis~
	fis~
	fis~
% Bars 21 to 25
	fis~
	fis~
	fis~_\crescmarkup
	fis~
	fis \stopMeasureCount
% Bars 26 to 30
	gis\ff\fermata \mark \default
	g'!4\f f!->~
	f8 r ees!4->~
	ees8 r d4->~
	d8 r c4->~
% Bars 31 to 35
	c8 r f,8.-> g16-.
	g8-. r g8.-> a16-.
	a8-. r a8.-> bes16-.
	bes8-. r bes8.-> c16-.
	c8-. r r4
% Bars 36 to 40
	R2*11
% Bars 41 to 45
	
% Bars 46 to 50
	
	\ni \mmrPos #-4 R2
	\mmrPos #-4 R
	\mmrPos #-4 R
	\mmrPos #-4 R \no
% Bars 51 to 55
	aes2\f~
	aes8 r r4
	R2*2
	
	g2\brack\f~
% Bars 56 to 60
	g8 r r4
	R2*6
% Bars 61 to 65
	
	\mark \default
	\repeat volta 2 {
		\mmrLength #12 \tempoXoffset -1 \mmrnDown R2*4
% Bars 66 to 70
		
		\ni \mmrPos #-4 R2
		\mmrPos #-4 R
		\mmrPos #-4 R \no
		\startMeasureCount ees2\p~
% Bars 71 to 75
		<< ees~ {s8\< s s s\!}>>
		<< ees2~ {s8\> s s s\!}>>
		ees2~
		ees \stopMeasureCount
		R2*3
% Bars 76 to 80
		
		
		r8 c8\p->~ c16.[ c32\< c16] r
		r8 ees!->~ ees16.[ ees32 ees16]\! r
		r8 g->~ g16.[ g32 g16] r
% Bars 81 to 85
		bes2\f\>
		a(\!
		f8) r r f\p~
		f16.[ f32 f16] r r8 f~
		f16.[ f32 f16]_\crescmarkup r r8 f~
% Bars 86 to 90
		f16.[ f32 f16] r r8 f
		aes16.\ff[ aes32 aes16] r r8 aes8~
		aes16.[ aes32 aes16] r r8 b~
		b16.[ b32 b16] r r8 d~
		d16.[ d32 d16] r r8 d
% Bars 91 to 95
		d!2\ff~
		d4 r
		R2*2
		
		e4\f r
% Bars 96 to 100
		R2
		e4 r
		R2
		ees!2~\ff
		ees4 r
% Bars 101 to 105
		R2*2
		
		\dynEO #'(0 . -8) f4^\f r
		R2
		\dynEO #'(0 . -7) f4^\f r
% Bars 106 to 110
		R2
		b,2\ff~
		b~
		b~
		b \mark \default
% Bars 111 to 115
		c~
		c~
		c8 r r4
		R2*9
% Bars 116 to 120
	
% Bars 121 to 125
		
		
		\ni \mmrPos #-4 R2
		\mmrPos #-4 R
		\mmrPos #-4 R
% Bars 126 to 130
		\mmrPos #-4 R \no
		f,2\pp~
		f~
		f~
		f
% Bars 131 to 135
		R2*4
		
		
		
		c2\pp~
% Bars 136 to 140
		c~
		c~
		c
		R2*5
		
% Bars 141 to 145
		
		
		
		\mmrLength #16 R2*14
% Bars 146 to 150
	
% Bars 151 to 155
	
% Bars 156 to 160
		
		
		\ni \mmrPos #-4 R2 
		\mmrPos #-4 R \no \mark \default
		f'2\f~
% Bars 161 to 165
		f8 c-. a-. c-.
		\tuplet 3/2 4 {f( c a} c16 d ees g)
		\tuplet 3/2 4 {f8( c a} c16 d ees g)
		f2~
		f8 c-. a-. c-.
% Bars 166 to 170
		\tuplet 3/2 4 {f8( c a} c16 d ees g)
		\tuplet 3/2 4 {f8( c a} c16 d ees g)
		fis2\ff~
		fis~
		fis~
% Bars 171 to 175
		fis~
		fis8 r r4
		R2*3
		
		
% Bars 176 to 180
		fis,4->\p eis->
		fis-> a->~
		a gis8. eis16
		eis4( fis8) r
		gis4 fisis
% Bars 181 to 185
		gis cis~
		cis b8. gis16
		gis4 r
		a\p-> gis->
		a-> c!~
% Bars 186 to 190
		c b8. gis16
		gis4( a8) r
		b4-> ais->
		b-> e->~
		e d8. b16
% Bars 191 to 195
		b2
		c4-> b->
		c-> ees->~
		ees d8. b16
		b4(_\crescmarkup c8) r
% Bars 196 to 200
		d4-> cis->
		d->\< g~
		g f8. d16
		d4\! r \mark \default
		d2\f~
% Bars 201 to 205
		d4 c->~
		c bes8.-> aes16-.
		g2->(
		f8) r aes'4\mf~
		aes2~
% Bars 206 to 210
		aes~
		aes~
		aes8 r d,4\brack\ff~
		d2~
		d~
% Bars 211 to 215
		d~
		d8 r d4~
		d2~
		d~
		d
% Bars 216 to 220
		\tuplet 3/2 4 {e8\f e e  e e e
		e e e  e e e}
		e4.\f( cis8)
		a4 d8.( cis16)
		\tuplet 3/2 4 {cis8 cis cis  cis cis cis
% Bars 221 to 225
		cis cis cis  cis cis cis}
		e!4.( cis8)
		a4 d8.-> cis16-.
		cis2~
		cis~
% Bars 226 to 230
		cis~
		cis
		cis8 r r4
		R2*7
% Bars 231 to 235
	
% Bars 236 to 240
		\alternative {
			\volta 1 {
				R2*2
			
			}
			\volta 2 {
				R2*2
				
			}
		}
	}
	R2*2
% Bars 241 to 245
	
	\ni \mmrPos #-4 R2
	\mmrPos #-4 R
	\mmrPos #-4 R
	\mmrPos #-4 R \no
% Bars 246 to 250
	b4->\mp b->
	b-> b->_\crescmarkup
	b-> b->
	b-> b->
	R2*8
% Bars 251 to 255
	
% Bars 256 to 260
	
	\mark \default
	d4->\pp d->
	d-> d->
	d-> d->
% Bars 261 to 265
	d-> d->
	d2~\pp
	d8 d d d
	d2~
	d8\< d d d
% Bars 266 to 270
	g2~
	g8 g g g
	f2~
	f8 f[ f f]\!
	e2\f~
% Bars 271 to 275
	e~
	e~
	e4 c8. e16
	e4 e8. d16
	c8 r e8._\dimmarkup d16
% Bars 276 to 280
	c8 r \once \tupletUp \tuplet 3/2 4 {e-.\> e-. d-.}
	c\! r \tuplet 3/2 4 {e\p-. e-. d-.}
	ees!8\pp r r4
	R2*3
	
% Bars 281 to 285
	
	c,2~
	c~
	c~
	<< c {s8\> s s s\!}>>
% Bars 286 to 290
	aes'2\p\<~
	aes\!
	<< aes2~ {s8\> s s s\!}>>
	aes2
	g8 r r4
% Bars 291 to 295
	R2*4
	
	
	
	\tuplet 3/2 4 {d'8(\f\< c g')} g4~\!
% Bars 296 to 300
	g2~
	g \mark \default
	cis,4(\f a)
	e'( a,8. a16)
	cis4( a8. b16)
% Bars 301 to 305
	cis2
	e4 cis8.( e16)
	g,2~
	g~\>
	<< g {s8 s s s\!}>>
% Bars 306 to 310
	c!8 r r4
	R2*5
	
	
	
% Bars 311 to 315
	
	\ni \mmrPos #-4 R2
	\mmrPos #-4 R \no
	c2\p~
	c~
% Bars 316 to 320
	c
	c~
	<< c~ {s4 s_\crescmarkup}>> 
	c2~
	c
% Bars 321 to 325
	c
	R2*8
	
	
	
% Bars 326 to 330
	
	
	
	\mark \default
	R2*14
% Bars 331 to 335
	
% Bars 336 to 340
	
% Bars 341 to 345
	
	
	
	\ni \mmrPos #-4 R2
	\mmrPos #-4 R \no
% Bars 346 to 350
	ges'!2(~\f\<
	ges4\! f8) r
	R2*8
	
	
% Bars 351 to 355
	
	
	
	
	\mark \default
% Bars 356 to 360
	R2*14
% Bars 361 to 365
	
% Bars 366 to 370
	
	
	
	
	\ni \mmrPos #-4 R2
% Bars 371 to 375
	\mmrPos #-4 R2 \no
	r4 r8. c16
	b4.->\f c8-.
	b4.-> c8-.
	b-. aes-. g-. f-.
% Bars 376 to 380
	ees r r8. bes'16
	d4.-> ees8-.
	d4.-> ees8-.
	d-. c-. bes-. aes-.
	g r r4
% Bars 381 to 385
	g2->
	f4-> g->
	ees2->
	bes'4..-> bes16
	bes2->\fz
% Bars 386 to 390
	aes4-> bes->
	ges2->\fz
	ees'8\f r r4
	e8 r r4 \mark #11
	R2
% Bars 391 to 395
	f,2~\fp\<
	<< f~ {s8 s s s\!}>>
	f2
	f'8-.\f g16( f) \tuplet 3/2 4 {f,8( e f)}
	f'8-. g16( f f,8) r
% Bars 396 to 400
	f'-. g16( f) \tuplet 3/2 4 {f,8( e f)}
	f'8-. g16( f f,8) r
	f'8-. g16( f) f8-. g16( f) 
	f8-. g16( f) f8-. g16( f) 
	f8-. g16( f) f8-. g16( f) 
% Bars 401 to 405
	f8-. g16( f) f( g f) r\fermata
	R2*14
	
	
	
% Bars 406 to 410
	
% Bars 411 to 415
	
% Bars 416 to 420
	\ni \mmrPos #-4 R2
	\mmrPos #-4 R
	\mmrPos #-4 R
	\mmrPos #-4 R \no
	bes,2~\pp
% Bars 421 to 425
	bes~
	bes\<
	d\!~
	d
	c~\fz
% Bars 426 to 430
	c4. r8
	R2*8
% Bars 431 to 435
	
	
	
	
	r8 c,->\p\<~ c16.[ c32 c16] r
% Bars 436 to 440
	r8 ees~-> ees16.[ ees32 ees16] r
	r8 g->~ g16.[ g32 g16]\! r
	\once \stemUp bes2(\fz\>
	<< a! {s8  s s s\!}>> \mark \default
	f8) r r f\p~
% Bars 441 to 445
	f16.[\< f32 f16] r r8 f~
	f16.[ f32 f16] r r8 f~
	f16.[ f32 f16] r r8 f\!
	aes16.[_\fcresc aes32 aes16] r r8 aes->~
	aes16.[ aes32 aes16] r r8 b->~
% Bars 446 to 450
	b16.[ b32 b16] r r8 d->~
	d16.[ d32 d16] r r8 d
	d2\f~
	d4 r
	R2*2
% Bars 451 to 455
	
	e4\f r
	R2
	e4 r
	R2
% Bars 456 to 460
	ees!2\ff~
	ees4 r
	R2*2
	
	f4\f r
% Bars 461 to 465
	R2
	f4\f r
	R2
	gis2~\ff
	gis~
% Bars 466 to 470
	gis~
	gis
	aes,(~\pp
	aes~
	aes~
% Bars 471 to 475
	aes \mark \default
	g8) r r4
	R2*5
% Bars 476 to 480
	
	
	g4-\offset X-offset -3 _\fzpocoapoco_crescendo~ g8 \tuplet 3/2 8 {g16( a b}
	c4.)->\< d8-.\!
	ees4.\fz d8
% Bars 481 to 485
	c-.\< g'-. g-. f-.\!
	ees4.->-\offset X-offset -1 _\piuf d8
	c8-.\< g'-. g-. f-.\!
	\dynEO #'(0 . -9.5) ees->^\f \tuplet 3/2 8 {d16( c b} c8) r
	c8-> \tuplet 3/2 8 {bes!16( aes g} aes8) r
% Bars 486 to 490
	aes8-> \tuplet 3/2 8 {g16( f e} f8) r
	f'8-> \tuplet 3/2 4 {ees!16( des c} des8) r
	R2*8
	
	
% Bars 491 to 495
	
% Bars 496 to 500
	r4 cis,\brack\pp
	r cis
	r d
	r d 
	\textMark "Vi-" R2*4
% Bars 501 to 505
	
	
	\mark \default
	\tuplet 3/2 4 {g'8(\f f! g} f4)
	\tuplet 3/2 4 {g8( f g} f4)
% Bars 506 to 510
	\tuplet 3/2 4 {g8( f g} f4)
	\tuplet 3/2 4 {g8( f g} f4)
	\tuplet 3/2 4 {g8( f g f g f)
	g( f g f g f)}
	g-. f16( g f g f g)
% Bars 511 to 515
	f8-. g16( f g f e d)
	cis2~
	cis~
	cis~
	cis
% Bars 516 to 520
	d8 r r4
	R2*7
% Bars 521 to 525
	
	
	
	a2-\offset X-offset -4 _\fzpocoapoco_crescendo~
	a
% Bars 526 to 530
	d\<~
	<< d {s4 s\!}>>
	f4.->\f e8-.
	d-. a'-. a-. g-. 
	f4.-> e8
% Bars 531 to 535
	d-. a'-. a-. g-. 
	f-. \tuplet 3/2 8 {e16( d cis} d8) r
	d-. \tuplet 3/2 8 {c!16( bes a } bes8) r
	bes-. \tuplet 3/2 8 {a16( g fis} g8) r
	g'-. \tuplet 3/2 4 {f!16( ees! d} ees8) r
% Bars 536 to 540
	R2*6
% Bars 541 to 545
	
	r4 r8 bes(\p\<
	\tuplet 3/2 4 {\once \tupletUp aes g ees'\!} ees4~\> \offset X-offset -2.3 \textEndMark "-de" \markWhiteout \mark \default
	ees8)\! r  ces4\pp
	r c!
% Bars 546 to 550
	R2*4
	
	
	
	\tuplet 3/2 4 {cis8(-\offset X-offset -1 \ff b cis} b4->)
% Bars 551 to 555
	\tuplet 3/2 4 {cis8( b cis} b4->)
	\tuplet 3/2 4 {cis8( b cis} b4->)
	\tuplet 3/2 4 {cis8( b cis} b4->)
	\tuplet 3/2 4 {cis8( b cis b cis b)
	cis( b cis b cis b)}
% Bars 556 to 560
	cis8-. b16( cis b cis d cis)
	d( cis b cis b a g fis)
	g'2\ff~
	g~
	g~
% Bars 561 to 565
	g
	f~
	f~
	f8 r r4
	R2
% Bars 566 to 570
	\startMeasureCount \tuplet 3/2 4 {c8\mf c c  c\> c c
	c c c  c c c\! \resetMarkSpace \mark \default
	c\p c c  c\> c c
	c c c  c c c\!
	c\pp c c  c c c 
% Bars 571 to 575
	c c c  c c c
	c c c^\calando  c c c
	c c c  c c c} \stopMeasureCount
	c4 r
	c\< r
% Bars 576 to 580
	c r
	c\! r
	d2\pp~
	d~
	d~
% Bars 581 to 585
	d
	d(~_\brackM\legato\<
	d4\! c\>->~
	c\! bes8. a16-.
	<< g2->) {\hairpinShorten #'(0 . -0.5) s8\> s s s\!}>>
% Bars 586 to 590
	f2\pp~
	f~
	f~
	f
	R2*5
% Bars 591 to 595
	
	
	
	
	\mmrLength #14 R2*12
% Bars 596 to 600
	
% Bars 601 to 605
	
% Bars 606 to 610
	
	\ni \mmrPos #-4 R2
	\mmrPos #-4 R
	\mmrPos #-4 R
	\mmrPos #-4 R \no \mark \default
% Bars 611 to 615
	bes'2\ff~
	bes8 f-. d-. f-.
	\tuplet 3/2 4 {bes( f d f aes c)
	bes( f d f aes c)}
	bes2~
% Bars 616 to 620
	bes8 f-. d-. f-.
	\tuplet 3/2 4 {bes8( f d f aes c
	bes f d f aes c)}
	d,\f r r4
	R2
% Bars 621 to 625
	ees8\f r r4
	R2
	e8\f r r4
	r r8. cis16\ff
	d2
% Bars 626 to 630
	c8( des c ees!)
	d!2
	c8( des c c)
	bes2->
	f'4.-> f8
% Bars 631 to 635
	f r ees[ r16 d]
	d8 r d4
	f2->
	f4. f8
	f r ees8. d16
% Bars 636 to 640
	d8 r r8. d16
	ees8 r d[ r16 c]
	c8 r r8. c16
	f8 r ees[ r16 d]
	d8 r r8. d16
% Bars 641 to 645
	e4 r8 cis
	d4 r8 f
	e4 r8 cis
	d4 r8 f
	ees! r r4
% Bars 646 to 650
	R2*6
% Bars 651 to 655
	
	d,2_\fpespr
	R
	e_\pespr( \mark \default
	\tempoXoffset 0.5 f8) r r4
% Bars 656 to 660
	R2*6
% Bars 661 to 665
	
	R2
	R2*4
% Bars 666 to 670
	
	\mmrLength #16 \tempoXoffset -2 R2*4
% Bars 671 to 675
	\mmrLength #12 R2*5
% Bars 676 to 680
	\mmrLength #10 \tempoXoffset -2 \mmrnDown R2*5
% Bars 681 to 685
	\ni \mmrPos #-4 R2
	\mmrPos #-4 R \no \markWhiteout \mark \default
	\voiceTwo r4\fermata r \oneVoice 
	cis4->\f d->
	c! fis
% Bars 686 to 690
	f! r
	a2~
	a8 a f ees
	d4 b'8. b16
	c4 c
% Bars 691 to 695
	bes!4. f8
	des'-. r d-. r
	bes r a8. a16
	f8 r e'8. e16
	ees2->~
% Bars 696 to 700
	ees8 ees-. d-. c-.
	bes r b8. b16
	c8 r f r
	ees2->~
	ees8 ees-. d-. c-.
% Bars 701 to 705
	bes r b8. b16
	c4-> f->
	c-> f->
	c-> f->
	a,\ff( f)
% Bars 706 to 710
	a2~
	a4 bes8. c16
	c4( bes)
	a( bes)
	a ees'\fz~
% Bars 711 to 715
	ees d8. bes16
	bes4 ees\fz~
	ees d8. bes16
	bes4 e->\fff~
	e2~
% Bars 716 to 720
	e~
	e
	f8 r r4
	R2*4
	
% Bars 721 to 725
	
	
	r4 c-.\ff
	bes-. g'-.
	f-. r
% Bars 726 to 729
	r f
	f r
	f r
	f r\fermata \fine
}
