%  work        : Symphony No. 2 in B-flat Major Op.4
%  typesetter  : Sébastien MANEN
%  date        : Friday 31 January 2025, 23:08
%###############################################################################
%#                          M U S I C   S E C T I O N                          #
%###############################################################################
musicFagottoIIMvtI = \relative c {
	\clef bass
	\key bes \major
%	\transposition a
% Bars 1 to 5
	\startMeasureCount bes,2\pp~
	bes~
	bes~
	bes~
	bes~
% Bars 6 to 10
	bes~
	bes~
	bes~
	bes~\crescD\<
	bes~
% Bars 11 to 15
	bes~
	bes\! \stopMeasureCount
	g'\ff\fermata
	\startMeasureCount d\pp~
	d~
% Bars 16 to 20
	d~
	d~
	d~
	d~
	d~
% Bars 21 to 25
	d~
	d~
	d~_\crescmarkup
	d~
	d \stopMeasureCount
% Bars 26 to 30
	b'\ff\fermata \mark \default
	f'!4\f g->~
	g8 r a4->~
	a8 r bes!4->~
	bes8 r c4~->
% Bars 31 to 35
	c8 r f,8.-> g16-.
	g8-. r g8.-> a16-.
	a8-. r a8.-> bes16-.
	bes8-. r bes8.-> c16-.
	c8-. r r4
% Bars 36 to 40
	R2*7
	
	
	
	
% Bars 41 to 45
	
	
	g8-.\f g,-. g'8.-> f16
	f8-.[ g,-.\<] f'-. ees-.
	ees-. g,-. ees'8.-> d16
% Bars 46 to 50
	d8[-. g,]-. c-. c'-.\!
	des,\f-. des,-. des'8.-> ces16-.
	ces8[-. des,]-. ces'-. bes-.
	bes-. des,-. bes'8.-> aes16-.
	aes8[-. des,]-. ges-. ges-.
% Bars 51 to 55
	des2\f~
	des8 r r4
	des''\p aes8. ees16
	f( ges aes bes aes8-.) f-.
	c,2-\offset X-offset -1.5 \ff~
% Bars 56 to 60
	c8 r r4
	c''\p g8. d16
	e( f g a! g8) r
	f2\pp~
	f~
% Bars 61 to 65
	<< f~ {s8\< s s s\!}>>
	<< f2 {s8\> s s s\!}>> \mark \default
	\repeat volta 2 {
		\tempoXoffset #0.5 bes,8 r r4
		R2*6
		
% Bars 66 to 70
		
		
		
		
		\startMeasureCount bes'2~\p
% Bars 71 to 75
		bes~
		bes~
		bes~\>
		bes\! \stopMeasureCount
		R2*3
% Bars 76 to 80
		
		\clef bass
		f8(\p e) r e([
		g\<-> f)] r f[(
		bes-> a)]\! r a
% Bars 81 to 85
		c(\f ees c bes)
		a(\> ges ees c)\!
		f r f'4\f~
		f ees8-.-> f-.->
		g r c,4->\fz~
% Bars 86 to 90
		c d8-.-> ees-.->
		b r r4
		R2*3
		
		
% Bars 91 to 95
		e,,8-.\ff[ b']-. gis-. e'-. 
		b-.[ gis'-.] e-.[ b'-.]
		d-. cis16( d) \tuplet 3/2 4 {b8( ais b)}
		d-. cis16( d b8) r
		d4->\ff b16[( cis b r32 a-.])
% Bars 96 to 100
		gis4-> fis16[( gis fis r32 e-.)]
		d4-> b16[( cis b r32 a-.])
		gis16( a gis fis) \tuplet 3/2 4 {gis8( fis e!)}
		f!8[-. c']-. a-. f'-. 
		c[-. a']-. f-. c'-. 
% Bars 101 to 105
		ees-. d16( ees) \tuplet 3/2 4 {c8( b c)}
		ees-. d16( ees c8) r
		ees4->\ff c16([ d c r32 bes-.)]
		a4-> g16([ a g r32 f-.)]
		ees4 c16([ d c r32 bes-.)]
% Bars 106 to 110
		a16( bes a g) \tuplet 3/2 4 {a8( g f)}
		f-\offset X-offset -1 \ff e f4~
		f8 g aes g
		f g aes4~
		aes8 g aes f \mark \default
% Bars 111 to 115
		c2~
		c
		e'\ff~
		e \clef tenor
		\tuplet 3/2 4 {c'8-\offset X-offset -1 \mf c c  c c c
% Bars 116 to 120
		c c c  c\> c c
		c c c  c c c\!}
		c\p c c\> c
		bes bes bes bes\!
		\tuplet 3/2 4 {bes bes bes\pp  bes bes bes
% Bars 121 to 125
		bes bes^\brackM\dimin bes  bes bes bes
		bes bes bes  bes bes bes~}
		bes r r4
		R2*3
		
% Bars 126 to 130
		\clef bass
		f,2\pp~
		f~
		f~
		f
% Bars 131 to 135
		R2*4
		
		
		
		f2~\p
% Bars 136 to 140
		f~
		f~
		f
		R2*5
		
% Bars 141 to 145
		
		
		
		g8\p f'4(\fz d8)
		g, f'4(\fz d8)
% Bars 146 to 150
		g, f'4(\fz d8)
		g, f'4(\fz d8)
		g, g'4(\fz c,8)
		g g'4(\fz c,8)
		g g'4(\fz c,8)
% Bars 151 to 155
		g g'4(\fz c,8)
		f2\p~
		<< f~ {s4-\offset X-offset -1 _\brackM\pocoapococresc s}>>
		f2~
		f
% Bars 156 to 160
		bes~\<
		bes~
		bes~
		<< bes {s8 s s s\!}>> \mark \default 
		f'2\f~
% Bars 161 to 165
		f8 c-. a-. c-. \clef tenor
		\tuplet 3/2 4 {f8( c a} c16 d ees g)
		\tuplet 3/2 4 {f8( c a} c16 d ees g)
		f2~
		f8 c-. a-. c-.
% Bars 166 to 170
		\tuplet 3/2 4 {f8( c a} c16 d ees g)
		\tuplet 3/2 4 {f8( c a} c16 d ees g) \clef bass
		ees,2\ff~
		ees~
		ees~
% Bars 171 to 175
		ees~
		ees8 r r4
		R2*3
		
		
% Bars 176 to 180
		b2\p~
		b~
		b~
		b
		gis~
% Bars 181 to 185
		gis~
		gis~
		gis
		fis~\p
		fis~
% Bars 186 to 190
		fis~
		fis
		\startMeasureCount f!~
		f~
		f~
% Bars 191 to 195
		f~
		f~
		f~
		f~
		f \stopMeasureCount
% Bars 196 to 200
		f'~-\offset X-offset 1.5 \brack\crescD\<
		f4 bes~
		bes2~
		<< bes {s8 s s s\!}>> \mark \default
		\startMeasureCount f2\f~
% Bars 201 to 205
		f~
		f~
		f~
		f~
		f~
% Bars 206 to 210
		f~
		f~
		f~
		f~
		f~
% Bars 211 to 215
		f~
		f~
		f~
		f~
		f \stopMeasureCount
% Bars 216 to 220
		\tuplet 3/2 4 {a,!8\f a a   a a a
		a a a  a a a}
		a'4.\f a8
		a4 d8.( cis16)
		\tuplet 3/2 4 {a8 a a  a a a
% Bars 221 to 225
		a a a   a a a}
		a4.\ff a8
		a4 d8.-> cis16-.
		e,,2~
		e~
% Bars 226 to 230
		e~
		e
		e''8-.\fp f16( e) \tuplet 3/2 4 {e,8( dis e)}
		e'-. \shape #'((0 . -0.3)(0 . 0.5)(0.5 . 0.5)(0.3 . -1)) Slur dis16( e e,8) r
		e'-. \shape #'((0 . -0.3)(0 . 0.5)(0.5 . 0.5)(0.3 . -1)) Slur dis16( e e,8) r
% Bars 231 to 235
		e'-. \shape #'((0 . -0.3)(0 . 0.5)(0.5 . 0.5)(0.3 . -1)) Slur dis16( e e,8) r
		\hairpinShorten #'(-0.5 . -0.5) f,2\fp\>~
		<< f~ {s4 s\brack\dimD}>>
		f2~
		f~
% Bars 236 to 240
		\alternative {
			\volta 1 {
				f8 r r4
				R2
			}
			\volta 2 {
				\startMeasureCount \noteHeadEsw #'(-2.5 . 0) f'2\pp~
				f~
			}
		}
	}
	f~
% Bars 241 to 245
	f~ 
	f~
	f~
	f~
	f \stopMeasureCount
% Bars 246 to 250
	R2*4
	
	
	
	aes2\pp~
% Bars 251 to 255
	aes
	\startMeasureCount aes~
	aes~
	aes~
	aes~
% Bars 256 to 260
	aes~
	aes \stopMeasureCount \mark \default
	R2*8
% Bars 261 to 265
	
% Bars 266 to 270
	g2\p\<
	g4 g
	g2
	g4 g\!
	g2\f~
% Bars 271 to 275
	g~
	g~
	g4 g8. g16
	g8 r e8. g16
	c8 r e,8._\dimmarkup g16
% Bars 276 to 280
	c8 r \tuplet 3/2 4 {\once \tupletUp e,-.\> e-. g-.}
	c\! r \tuplet 3/2 4 {e,-.\p e-. g-.}
	aes!2\pp~
	aes~
	aes~
% Bars 281 to 285
	aes
	aes,!2\pp~
	aes~
	aes~
	<< aes {s8\> s s s}>>
% Bars 286 to 290
	<< c2 {\hairpinShorten #'(-0.5 . -1.5) s8\p\< s s s\!}>>
	des2
	<< ees {s8\> s s s\!} >>
	fes2
	\startMeasureCount e!~
% Bars 291 to 295
	e~\<
	e~\!
	e~
	e~_\crescmarkup
	e~
% Bars 296 to 300
	e~\f
	\noteHeadEsw #'(0 . 1.9) e \stopMeasureCount \mark \default
	a(\f
	cis~
	cis4) a8.( b16)
% Bars 301 to 305
	cis2
	e,4 cis8.( e16)
	g2~
	g~\>
	<< g {s8 s s s\!}>>
% Bars 306 to 310
	e2\p(
	f
	g
	a!)
	e(
% Bars 311 to 315
	f
	g
	aes)
	c,8-. d16( c \tuplet 3/2 4 {bes8 a! bes}
	a4. d8)
% Bars 316 to 320
	c-. d16( c \tuplet 3/2 4 {bes8 a bes}
	a4. d8)
	c-. d16( c bes_\crescmarkup a bes c
	a8) a r d
	c-. d16( c bes a bes c
% Bars 321 to 325
	a8) a r a
	bes,2\pp~
	bes~
	bes~
	bes
% Bars 326 to 330
	R2*4
	
	
	\markXoffset #-0.2 \mark \default
	a'2\pp~
% Bars 331 to 335
	a~
	a~
	a
	aes8^\solo-. d!-. aes-. d-. 
	aes-. aes-. d-. d-. 
% Bars 336 to 340
	aes-. d-. aes-. d-. 
	aes-. aes-. d-. d-. 
	g,2~\pp
	g~
	g~
% Bars 341 to 345
	g
	\shape #'((0 . 0)(0 . 0.5)(0 . 0.5)(0 . 0)) Slur fis4(\p\< gis
	ais b)\!
	cis( dis\<
	e fis)\!
% Bars 346 to 350
	aes(\f\< bes!)
	c(\! des8) r
	R2*4
	
	
% Bars 351 to 355
	
	r4 bes16(\f c bes aes
	g8) r r4
	r bes16(\mp c bes aes
	g8) r r4 \mark \default
% Bars 356 to 360
	r g,8.\p g16
	g8. g16 g8 r
	r4 f8. f16
	f8. f16 f8 r
	r4 aes8. aes16
% Bars 361 to 365
	aes8. aes16 aes8 r
	r4 g8. g16
	g8. g16 g8 r
	r4 bes8. bes16
	bes8. bes16 bes8 r
% Bars 366 to 370
	r4 a8. a16
	a8. a16 a8 r
	R2*4
	
	
% Bars 371 to 375
	
	r4 r8. c16
	d4.\f c8-.
	d4.-> c8-.
	d-. c-. bes-. aes-. 
% Bars 376 to 380
	g r r8. ees'16
	f4.-> ees8-.
	f4.-> ees8-.
	f-. ees-. d-. c-. 
	b r r4 \clef tenor
% Bars 381 to 385
	g''2->\f
	f4-> g->
	ees2->
	bes'4..-> bes16
	bes2\brack\fz
% Bars 386 to 390
	aes4-> bes->
	ges2->\brack\fz \clef bass
	g,,!8\f r r4
	ges8 r r4 \markXoffset #-0.2 \mark #11
	R2
% Bars 391 to 395
	r4 f\p\<~
	<< f2~ {s8 s s s\!}>>
	f2
	\tuplet 3/2 4 {f8\f-. g-. a-. bes-. c-. d-.}
	ees4.-> d8-.
% Bars 396 to 400
	c'-. bes-. a-. bes-. 
	c4.-> d8-.
	ees-. d-. c-. bes-. 
	a4.-> bes8-.
	c-. d-. ees-. d-. 
% Bars 401 to 405
	c-. bes-. a4\fermata
	R2*4
	
	
	
% Bars 406 to 410
	r4 g8\brack\p g
	r4 d8 d
	R2*4
	
	
% Bars 411 to 415
	
	\startMeasureCount ees,2\pp~
	ees~
	ees~
	ees~
% Bars 416 to 420
	ees~\pp
	ees~
	ees~
	ees \stopMeasureCount
	bes'8 r r4
% Bars 421 to 425
	R2*11
	
	
	
	
% Bars 426 to 430
	
	
% Bars 431 to 435
	
	\ni \mmrPos #-4 R2
	\mmrPos #-4 R
	\mmrPos #-4 R \no
	f'8(\p\< e) r e[(
% Bars 436 to 440
	g-> f)] r f([
	bes-> a)]\! r a
	c(\f\> ees c bes)
	a( ges ees c)\! \markXoffset #-0.2 \mark \default
	f r f'4->\f~
% Bars 441 to 445
	f\< ees8-.-> f-.->
	g r c,4->~
	c d8-.-> ees-.->\!
	b, r r4
	R2*3
% Bars 446 to 450
	
	
	e,8[\ff-> b']-> gis-> e'->
	b gis' e b'
	d( cis16 d) \tuplet 3/2 4 {b8( ais b)}
% Bars 451 to 455
	d-. cis16( d b8) r
	d4-> b16[( cis b r32 a)]-.  % bes => b based on bass voice
	gis4-> fis16[( gis fis r32 e-.)]
	d4-> b16[( cis b r32 a-.)]
	gis16( a gis fis) \tuplet 3/2 4 {gis8( fis e)}
% Bars 456 to 460
	f!8\ff[ c'!] a f'
	c[ a'] f c'
	ees!-. d16( ees) \tuplet 3/2 4 {c8( b c)}
	ees-. d16( ees c8) r
	ees4-> c16([ d c r32 bes-.)]
% Bars 461 to 465
	a4-> g16([ a g r32 f-.)]
	ees4-> c16([ d c r32 bes-.)]
	a16( bes a g) \tuplet 3/2 4 {a8( g f)}
	f'2\ff~
	f~
% Bars 466 to 470
	f~
	f
	ges8 r r4
	R2*3
	
% Bars 471 to 475
	\mark \default
	R2*4
% Bars 476 to 480
	\ni \mmrPos #-4 R2
	\mmrPos #-4 R \no
	g4~_\fzpocoapococrescendoC g8 \tuplet 3/2 8 {\once \tupletUp g16( a! b}
	c4.->)\< d8-.\!
	ees4.\fz d8
% Bars 481 to 485
	c8\<-. g'-. g-. f\!-. 
	ees4._\piuf-> d8
	c\<-. g'-. g-. f\!-. 
	c-.\p[ g-.] c,-. g-.\fz
	aes-.[\p ees'-.] aes-. ees'-.\fz
% Bars 486 to 490
	f-.[\p c-.] f,-. c-.\fz
	f'8-> \tuplet 3/2 8 {ees!16( des c} des8) r
	R2*7
% Bars 491 to 495
	
	
	
	
	\ni \mmrPos #-4 R2 \no
% Bars 496 to 500
	r4 fis,\brack\pp
	r fis
	r g
	r g 
	\textMark "Vi-" R2
% Bars 501 to 505
	aes,4\brack\p a~
	a bes~
	bes b \mark \default
	\tuplet 3/2 4 {g'!8(\f f g} f4)
	\tuplet 3/2 4 {g8( f g} f4)
% Bars 506 to 510
	\tuplet 3/2 4 {g8( f g} f4)
	\tuplet 3/2 4 {g8( f g} f4)
	\tuplet 3/2 4 {g8( f g f g f)
	g( f g f g f)}
	g-. f16( g f g f g)
% Bars 511 to 515
	f8-. g16( f g f g f)
	e2~
	e~
	e~
	e
% Bars 516 to 520
	f8 r r4
	R2*18
% Bars 521 to 525
	
% Bars 526 to 530
	
% Bars 531 to 535
	
	
	
	
	\ni \mmrPos #-4 R2
% Bars 536 to 540
	\mmrPos #-4 R \no
	\tuplet 3/2 4 {g8-\offset X-offset -1 \fp g g  g g g}
	g r r4
	\tuplet 3/2 4 {g8-\offset X-offset -1 \fp g g  g g g}
	g r r4
% Bars 541 to 545
	\tuplet 3/2 4 {g8-\offset X-offset -1 \fp g g  g g g}
	g r r4
	\tuplet 3/2 4 {g8-\offset X-offset -1 \fp g g  g g g} \textEndMark "-de" \mark \default
	g8 r aes4\pp
	r a!
% Bars 546 to 550
	R2*4
	
	
	
	\tuplet 3/2 4 {a8(-\offset X-offset -1 \ff g! a} g4)->
% Bars 551 to 555
	\tuplet 3/2 4 {a8( g a} g4->)
	\tuplet 3/2 4 {a8( g a} g4->)
	\tuplet 3/2 4 {a8( g a} g4->)
	\tuplet 3/2 4 {a8( g a g a g)
	a( g a g a g)}
% Bars 556 to 560
	a-. g16( a g a g a)
	g8-. a,16( b cis d e fis)
	g2\ff~
	g~
	g~
% Bars 561 to 565
	g
	a~
	a~
	a8 r r4
	R2
% Bars 566 to 570
	\tuplet 3/2 4 {\startMeasureCount c,8\mf c c  c\> c c
	c c c  c c c\! \mark \default
	c\p c c  c\> c c
	c c c  c c c\!
	c\pp c c  c c c
% Bars 571 to 575
	c c c  c c c
	c c c^\brackM\calando  c c c
	c c c  c c c } \stopMeasureCount
	bes4 r
	bes\< r
% Bars 576 to 580
	bes r
	bes\! r
	bes,2-\offset X-offset -2.3 -\tweak extra-offset #'(-0.3 . 0.3) \pp~
	bes~
	bes~
% Bars 581 to 585
	bes~
	bes8 r r4
	R2*3
	
	
% Bars 586 to 590
	bes2~\p
	bes~
	bes~
	bes
	bes''2\p(~
% Bars 591 to 595
	bes4 c->~\<
	c d8.\! f16-.
	cis2\>
	d8)\! r r4
	c,!2\p~
% Bars 596 to 600
	c
	c~
	c
	<< c~ {s4 s\< }>>
	c2~
% Bars 601 to 605
	c~
	<< c {s8 s s s\!}>>
	bes'2\mf~
	bes~\<
	<< bes~ {s8 s s s\!}>>
% Bars 606 to 610
	bes2
	a\mf~
	a
	a\brack\f\<
	<< c {s8 s s s\!}>> \mark \default
% Bars 611 to 615
	bes2\ff~
	bes8 f-. d-. f-.
	\tuplet 3/2 4 {bes( f d f aes c)
	bes( f d f aes c)}
	bes2~
% Bars 616 to 620
	bes8 f-. d-. f-.
	\tuplet 3/2 4 {bes( f d f aes c
	bes f d f aes c)}
	gis\f r r4
	R2
% Bars 621 to 625
	a8\f r r4
	R2
	bes8\f r r4
	r4 r8. bes16\ff
	f2~
% Bars 626 to 630
	f~
	f~
	f
	d2->
	c4.-> c8
% Bars 631 to 635
	bes r bes[ r16 bes]
	bes8 r bes4
	d2
	c
	bes8 r bes8. bes16
% Bars 636 to 640
	bes8 r r8. bes16
	bes8 r bes[ r16 bes]
	bes8 r r8. bes16
	bes8 r bes[ r16 bes]
	bes8 r r8. bes16
% Bars 641 to 645
	bes4 g
	aes f
	bes g
	aes f
	g gis
% Bars 646 to 650
	\shape #'((0 . 2)(0 . -0.3)(0.5 . -0.6)(0 . 0)) Slur a( bes
	c cis
	d) ees(
	f fis
	g c,
% Bars 651 to 655
	f!8) r r4
	bes2_\fpespr
	R2
	bes(_\pespr \markWhiteout \mark \default
	\tempoXoffset #0.5 a8) r r4
% Bars 656 to 660
	R2*11
	
	
	
	
% Bars 661 to 665
	
% Bars 666 to 670
	
	f2\pp(
	d~
	d)
	c
% Bars 671 to 675
	d\pp~
	d~
	d~
	d4. r8
	R2
% Bars 676 to 680
	\mmrLength #11 \mmrnDown \tempoXoffset #-2 R2*6
	
	
	
	
% Bars 681 to 685
	
	f2\pp(~ \markWhiteout \mark \default
	f4\fermata bes) \clef bass
	g4\f f
	e ees
% Bars 686 to 690
	d des
	c b
	bes4 f8 f
	g4 gis
	a f
% Bars 691 to 695
	bes bes8. aes16
	g8-. r f-. r
	e r ees[ r16 ees]
	d'8 r des8. des16
	c4( b)
% Bars 696 to 700
	c( f,)
	g( gis)
	a( f)
	c'( b)
	c( f,)
% Bars 701 to 705
	g( gis)
	a-> f->
	a-> f->
	a-> f->
	ees(\ff d)
% Bars 706 to 710
	ees' c->~
	c bes8. a16
	a4( bes)
	c( bes)
	c f,->\fz~
% Bars 711 to 715
	f f8. bes16
	bes4 f\fz~
	f f8. bes16
	bes4 g\fff~
	g g~
% Bars 716 to 720
	g g~
	g g
	f8 r r8. f16
	g4-. gis-.
	a-. bes-.
% Bars 721 to 725
	c,-. cis-.
	d-. ees-.
	f-. fis-.
	g-. c,-.
	f!-. r
% Bars 726 to 729
	r f
	bes r
	bes r
	bes, r\fermata \fine
}
